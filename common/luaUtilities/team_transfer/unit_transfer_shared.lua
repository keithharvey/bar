...
---Empty an array in place so a lifted result table can be reused across calls without
---churning garbage. Lua has no cheap array clear, so we nil from the tail.
---@param arr table
...
function Shared.ValidateUnits(policyResult, unitIds, springApi, unitDefs, out)
  ...
  -- Reuse the caller's result table when supplied; otherwise allocate once. Either way
  -- the scalar fields are reassigned and the arrays cleared so stale entries never leak.
  out = out or {}
  ...
  -- Name dedupe sets stay call-local: they are keyed by the names we are about to collect,
  -- so a shared/lifted one would leak keys across rotating `out` tables (memo reuse).
