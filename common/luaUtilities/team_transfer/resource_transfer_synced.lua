...
local function rejectPolicy(ctx, resourceType)
  return {
    ...
    cumulativeSent = Shared.GetCumulativeSent(ctx.senderTeamId, resourceType),
...
local function tryRejectPolicy(ctx, resourceType)
  ...
  local activePlayers = tonumber(ctx.springRepo.GetTeamRulesParam(ctx.receiverTeamId, "numActivePlayers")) or 0
