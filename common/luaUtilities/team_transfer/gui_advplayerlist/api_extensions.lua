...
function API.HandleHoverChange(myTeamID, selectedUnits, newHoverTeamID, newHoverPlayerID)
  ...
  if newHoverTeamID and selectedUnits and #selectedUnits > 0 then
    ...
    if not policyResult.canShare and Spring.AreTeamsAllied(myTeamID, newHoverTeamID) then
      -- Sharing is fully disabled for this ally under the current mode (e.g. Tech
      -- Core at a tier whose unit_sharing_mode is "none"), so every selected unit is
      -- un-shareable. ValidateUnits short-circuits to an empty list when canShare is
      -- false, so flag them all here for the hover highlight.
