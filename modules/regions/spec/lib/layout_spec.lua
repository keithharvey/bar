	...
	it("keeps a curve's anchors and a rect's corners, and derives the outline and the kind on the way back", function()
		local layout = Layout.Export({
			...
			},
			{
				...
				team = 2,
				kind = "box",
				vertices = { { x = 100, z = 100 }, { x = 150, z = 100 }, { x = 150, z = 150 }, { x = 100, z = 150 } },
			},
		}, byKey, 200, 200)
		assert.are.same({ x = 0, y = 0, strength = 0.5 }, layout.regions.start[1].poly[1], "strength snapped to 0.025")
		assert.is_nil(layout.regions.start[1].poly[2].strength)
		assert.are.same(
			{ { x = 100, y = 100 }, { x = 150, y = 150 } },
			layout.regions.start[2].poly,
			"a rect is its two corners"
		)
		local back = assert(Layout.Parse(layout, byKey.start, 200, 200))
		assert.are.equal("spline", back[1].kind)
		assert.are.equal(0.5, back[1].controls[1].strength)
		assert.is_true(#back[1].vertices > 4)
		assert.are.equal("box", back[2].kind)
