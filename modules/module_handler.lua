		...
		if errors[name] == nil then
			...
			for _, required in ipairs(manifests[name].requires) do
				if not manifests[required] then
					errors[name] = string.format("Module %q requires missing module %q; not loaded", name, required)
					break
				elseif loadFailureFor(required) then
					errors[name] = string.format("Module %q requires %q, which is not loaded", name, required)
					break
				end
			end
		end
...
function ModuleHandler.LoadActions(name, vfsMode)
	...
	local manifest = ModuleHandler.DiscoverManifests(vfsMode)[name]
...
function ModuleHandler.ModeVerbs(category, vfsMode)
	...
	local names = {}
	for name in pairs(ModuleHandler.DiscoverManifests(vfsMode)) do
		names[#names + 1] = name
	end
	table.sort(names)
	for _, name in ipairs(names) do
		local filePath = ModuleHandler.DiscoverManifests(vfsMode)[name].dir .. LAYOUT.modeVerbs
