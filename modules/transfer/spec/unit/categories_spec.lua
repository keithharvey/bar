local Categories = require("modules/transfer/unit/categories")
...
describe("unit_sharing_categories #categories", function()
	describe("classifyUnitDef", function()
		it("should classify commanders as Commander", function()
			...
			assert.equal(ConstructionEnums.UnitType.Commander, Categories.classifyUnitDef(def))
		end)

		it("should classify air transports as Combat", function()
			local def = { canFly = true, transportCapacity = 8, customParams = {} }
			assert.equal(ConstructionEnums.UnitType.Combat, Categories.classifyUnitDef(def))
		end)
		...
		it("should classify mobile constructors as Constructor", function()
			...
			assert.equal(ConstructionEnums.UnitType.Constructor, Categories.classifyUnitDef(def))
		end)
		...
		it("should classify factories as Factory", function()
			...
			assert.equal(ConstructionEnums.UnitType.Factory, Categories.classifyUnitDef(def))
		end)

		it("should classify energy buildings as Resource", function()
			local def = { customParams = { unitgroup = "energy" } }
			assert.equal(ConstructionEnums.UnitType.Resource, Categories.classifyUnitDef(def))
		end)
		...
		it("should classify util buildings as Utility", function()
			...
			assert.equal(ConstructionEnums.UnitType.Utility, Categories.classifyUnitDef(def))
		end)
	...
	describe("isTransportDef", function()
		it("should return true for flying transports (any tech)", function()
			assert.is_true(
				Categories.isTransportDef({ canFly = true, transportCapacity = 8, customParams = { techlevel = "2" } })
			)
		end)
