...
describe("the spot holder fact", function()
	...
	it("survives the wire: the deal decodes to what was encoded", function()
		local d = deal(fourTeams, sevenRegions)
		local back = assert(Deal.Decode(Deal.Encode(sevenRegions, d.regions, 200, 200), 200, 200))
		assert.are.same(d.regions, back.holders)
		assert.are.equal(#sevenRegions, #back.regions)
		local given, read = assert(sevenRegions[1]), assert(back.regions[1])
		assert.are.same(given.vertices, read.vertices)
		assert.are.equal(given.id, read.id, "the layout carries the ids the holders are keyed by")
		assert.is_nil(Deal.Decode("", 200, 200))
