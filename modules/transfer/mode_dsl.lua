...
M.MexSplitting = EconomyEnums.MexSplitting
...
local verbs = {
	...
	MexSplitting = rule(function(name, which)
		ModeBuilder.OneOf(name, "MexSplitting", EconomyEnums.MexSplitting, which)
		return { which = which }
	end, function(p, lock)
		return { [EconomyEnums.ModOptions.MexSplitting] = { value = p.which, locked = lock.noun, ui = p.ui } }
	end),
}
