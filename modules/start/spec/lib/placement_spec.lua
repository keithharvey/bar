...
describe("start placement", function()
	it("lays a count of positions around a circle, clamped to the map", function()
		...
		assert.are.equal(0, assert(pts[3]).x, "the point past the west edge is clamped")
	end)
	...
	it("takes a polygon's vertices first and then its edge midpoints", function()
		...
		local mid = assert(pts[2])
		assert.is_true(math.abs(mid.x - 550) < 1e-6 and math.abs(mid.z - 550) < 1e-6, "a midpoint")
	end)

	it("scatters random positions inside the circle", function()
		local seq = { 0.25, 0.5, 0.75, 1 - 1e-9 }
		local i = 0
		local pts = Placement.Random(
			500,
			500,
			{ shape = "circle", radius = 100, count = 2, rotation = 0 },
			1000,
			1000,
			function()
				i = i + 1
