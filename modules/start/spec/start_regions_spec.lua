...
describe("a map's starts, as a set", function()
	...
	it("is a rule for starts alone", function()
		---@return Region
		local mex = function(team, x)
			local region = area(team, x, 0, 100) --[[@as table]]
			region.type = Enums.Types.MexRegion
			region.group = "g"
			return region
		end
		assert.are.same({}, Regions.CheckSet(Enums.Types.MexRegion, { mex(1, 0), mex(2, 50) }))
	end)
end)
