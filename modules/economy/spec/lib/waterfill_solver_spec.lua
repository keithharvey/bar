...
local TAX_RATE_OPTION = "spec_tax_rate"
local BarEconomy = require("modules/economy/lib/waterfill_solver")
...
local function flowFor(flows, teamId, resourceType)
	...
	assert(
		summary,
		string.format("missing flow summary for team %s resource %s", tostring(teamId), tostring(resourceType))
	)
	return summary
end
...
---@return number
local function taxRateFor(springRepo)
	local rate = tonumber(springRepo.GetModOptions()[TAX_RATE_OPTION]) or 0
	return math.max(0, math.min(1, rate))
end

local function modOptions(opts)
	return {
		[TAX_RATE_OPTION] = opts.taxRate or 0,
	}
end
...
describe("Bar economy waterfill", function()
	it("balances overflow without tax", function()
		...
		local _, flows = BarEconomy.Solve(spring, teamsList, taxRateFor)
		...
		local aFlow = flowFor(flows, teamA.id, ResourceTypes.METAL)
		local bFlow = flowFor(flows, teamB.id, ResourceTypes.METAL)
		local cFlow = flowFor(flows, teamC.id, ResourceTypes.METAL)
	...
	local function metalResult(results, teamId)
		for _, result in ipairs(results) do
			if result.teamId == teamId and result.resourceType == ResourceTypes.METAL then
				return result
			end
		end
	...
	it("emits deltas that conserve flows across the group", function()
		...
		local teamsList = buildTeams({ teamA, teamB, teamC })
		local results = BarEconomy.SolveToResults(spring, teamsList, taxRateFor)
	...
	it("retains accumulated excess for a neutral team with storage headroom", function()
		local teamA =
			Builders.Team:new():WithMetal(500):WithMetalStorage(1000):WithMetalShareSlider(50):WithMetalExcess(300)
	...
	it("wastes excess beyond storage headroom without phantom sends", function()
		local teamA =
			Builders.Team:new():WithMetal(900):WithMetalStorage(1000):WithMetalShareSlider(50):WithMetalExcess(300)
