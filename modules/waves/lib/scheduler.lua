...
local function slowTick(spec, state, world, orders)
	...
	if spec.boss ~= nil and Boss.IsDue(bossAnger, burrowCount, t, params.gracePeriod)
		and Boss.CanSpawnMore(state.boss, spec.boss) then
	...
	then
		...
		emit(orders, {
			kind = "wave",
			ceiling = Anger.WaveCeiling(params, anger.techAnger, state.boss.spawned) * shape.sizeMultiplier * scale.size,
		})
	end
