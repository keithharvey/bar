--- The director: one named wave engine, spec in, orders out.
---
--- The gadget hosts 1..n of these, keyed by name. Nothing here touches
--- Spring — the world arrives as a table of readings and closures, so the
--- whole lifecycle (grace, first wave, the boss, an endless reloop) specs
--- under busted with a seeded RNG and a fake map.
---
--- The spec is IMMUTABLE after New. Everything the monolith mutated on its
--- config lives in `state.params`, a copy made here, and progress lives in
--- `state` — which is exactly the pile a savegame writes.
...
function Director.New(spec, rng)
	...
	---What the composer needs that is not in the spec.
	...
	---One burrow's extra squad, outside the wave cadence: what a fresh burrow
	---throws out, and what the whole map throws out when the boss lands.
	---@param world WaveWorld
	...
	---A named spawn straight to the queue — the minion path.
	---@param burrowID integer
	---@param defName UnitDefName
	---@param count integer
	director.ComposeNamed = function(burrowID, defName, count, rng)
		local state = director.state
		enqueue(Composer.ComposeNamed(burrowID, defName, count, spec.teamID, state.params.spawnChance, state.waveNumber, rng))
	end
	...
	---The mission dial. 1.0 is the spec's own pace; 0.3 is background
	---pressure; 2.0 is a siege. State, so it serializes.
	...
	---Force the next wave to be something specific — the scripted spike. The
	---override is consumed by one wave and then the wheel takes over again.
	...
	---Endless mode: the boss is dead, the ladder moves, the clocks reset.
