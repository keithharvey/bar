...
--- A director driven over a fake map: burrows that never move, ground that is
--- always land, and a seeded RNG. The whole lifecycle — grace, first wave,
--- the boss, an endless reloop — is reachable without Spring.
...
---Run frames [from, to] and collect every order the director gave.
---
---`drain` makes the harness a miniature gadget: it obeys the drain order the
---way the real one does, which is what lets the wave cadence advance at all —
---a wave never starts while the last one is still queued.
...
describe("waves director", function()
	describe("start", function()
		...
		it("flattens populations into a stable order, because pairs order is not a wire value", function()
			...
			assert.are.same({ "alpha", "mid", "zeta" }, {
				indexed.commanders[1].def, indexed.commanders[2].def, indexed.commanders[3].def,
			})
	...
	describe("waves", function()
		...
		it("holds off while the previous wave is still queued", function()
			...
			-- Nothing drains the queue here, so the cadence must stall — waves
			-- never overlap into one continuous stream.
	...
	describe("the endless reloop", function()
		it("resets the clocks and replaces the params table", function()
			...
			-- makeSpec({params=nil}) keeps the defaults; drive some progress first.
