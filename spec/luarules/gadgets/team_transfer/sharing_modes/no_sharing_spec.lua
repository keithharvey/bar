...
describe("No Sharing Configuration", function()
    ...
    describe("Allied Actions", function()
        it("should allow assist ally actions at policy level", function()
            local result = service:GetResult(sender.id, receiver.id)

            -- Assist ally policy always allows at policy level, restrictions apply at validator level
            if result.guard_transfer then
                assert.equal(true, result.guard_transfer.allowGuardCommands)
            end
            if result.repair_transfer then
                assert.equal(true, result.repair_transfer.allowRepairCommands)
            end
        end)
