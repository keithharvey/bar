...
local defaultData = {
	...
	units = {},
	players = {},
	metal = ResourceDataBuilder.new():WithCurrent(1000):WithStorage(1000):WithShareSlider(100):WithResourceType("metal"):Build(),
	energy = ResourceDataBuilder.new():WithCurrent(1000):WithStorage(1000):WithShareSlider(100):WithResourceType("energy"):Build(),
}

local nextTeamId = sequence.sequence("team_id", {
	start = 0,
	format = function(_, n)
		return n
	end,
})
local nextUnitId = sequence.sequence("unit_id", {
	start = 1,
	format = function(p, n)
		return tostring(n)
	end,
...
function TeamBuilder:WithUnit(unitDefID, unitIdCallback)
	...
	-- Create wrapper object with just the unitDefId - real data populated later by SpringSyncedBuilder
