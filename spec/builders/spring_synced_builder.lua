...
function SB:WithAlliance(team1ID, team2ID, isAllied)
	if isAllied == nil then
		isAllied = true
	end -- Default to allied for backward compatibility
...
function SB:BuildSpring()
			...
			if builtTeam.units then
				for _, unitWrapper in pairs(builtTeam.units) do
					...
					if not unitDef and defKey and instance._globalUnitDefNames then
						local info = instance._globalUnitDefNames[defKey --[[@as string]]]
						local numericId = info and info.id
						if numericId then
	...
	local function ensureTeam(teamID)
		...
		if not teamBuilder then
			error(string.format("TeamBuilder not found for teamID %d. Use SpringSyncedBuilder:WithTeam(teamBuilder) to configure teams properly.", teamID))
		end
		return teamBuilder
	end

	local function normalizeResourceType(resourceType)
		if resourceType == ResourceTypes.METAL then
			return ResourceTypes.METAL
		end
		if resourceType == ResourceTypes.ENERGY then
	...
	local function getResourceStore(teamID, resourceType)
		...
		if normalized == ResourceTypes.METAL then
	...
	local mock = {
		CMD = (Spring and (Spring --[[@as table<string, any>]]).CMD) or {
			LOAD_ONTO = 1,
			SELFD = 2,
			GUARD = 25,
			REPAIR = 40,
		...
		GetTeamResources = function(teamID, resourceType)
			...
			return data.current, data.storage, data.pull or 0, data.income or 0, data.expense or 0, data.shareSlider, data.sent, data.received, data.excess
		end,
		...
		GetUnitDefID = function(unitID)
			for _, teamBuilder in pairs(builtTeams) do
				if teamBuilder.units then
		...
		GetUnitIsBeingBuilt = function(unitID)
			for _, teamData in pairs(builtTeams) do
				if teamData.units and teamData.units[unitID] then
					local wrapper = teamData.units[unitID]
					return wrapper.beingBuilt or false, wrapper.buildProgress or 1.0
				end
			end
			return false, 1.0
		end,
		...
		TransferUnit = function(unitID, newTeamID, given)
			...
			if builtTeams[currentTeamID] and builtTeams[currentTeamID].units then
				builtTeams[currentTeamID].units[unitID] = nil
			end
			...
			builtTeams[newTeamID].units[unitID] = { unitDefId = unitDefID --[[@as integer|string]] }

			return true
		end,

		AreTeamsAllied = function(team1ID, team2ID)
		...
		SetTeamShareLevel = function(teamID, resource, level)
			local teamData = builtTeams[teamID]
			if teamData then
		...
		GetTeamAllyTeamID = function(teamID)
			local teamData = builtTeams[teamID]
			if teamData then
				return teamData.allyTeam or teamID
			end
			return nil
		end,

		GetPlayerListUnpacked = function()
			local result = {}
			for _, teamData in pairs(builtTeams) do
				result[#result + 1] = teamData
			end
			if #result == 0 then
				return nil
			end
...
function SB:WithGlobalsDefined(fn, persist)
	...
	---@diagnostic disable: global-in-non-module -- deliberate test-env global setup
	...
	_G.next = next
	_G.require = require
	---@diagnostic enable: global-in-non-module
