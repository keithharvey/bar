...
local function getUnitDefRequireModoptionDefaults()
    return {
        ...
        -- Unit sharing (added for tests)
        unit_sharing_mode = "enabled",

        -- Assist drones build power multiplier
        assistdronesbuildpowermultiplier = 1,

        -- Game speed (used in unitdefs_post)
        gamespeed = 30,
    }
end
...
function SpringRepositoryBuilder.new()
    return setmetatable({
        modOptions = {}, -- Start empty, defaults provided by GetModOptions when needed
...
function SpringRepositoryBuilder:BuildSpringRepository()
    ...
    local springRepo = {
        ...
        GetTeamResourcesUnpacked = function(self, teamID, resourceType)
            return getTeamResourceData(teamID, resourceType)
        end,
    ...
    springRepo.GetPlayerListUnpacked = function()
        -- Return TeamInfo objects for easier handling
        return springRepo.GetTeamList()
    end
    ...
    springRepo.TransferUnit = function(self, unitID, newTeamID, given)
        ...
        if currentTeamID == newTeamID then
            return true -- Already on correct team
        end
    ...
    springRepo.IsCheatingEnabled = function()
        return false -- Default to no cheating in tests
    end
...
function SpringRepositoryBuilder:WithRealUnitDefs()
    if not self._globalUnitDefs then
        ...
        end, true) -- persist=true to keep globals available
    end
    return self
end
