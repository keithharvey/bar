...
local function UnitDetected(unitID, unitDefID, unitTeam, noUpload)
	if unitDefRings[unitDefID] == nil then
		return
	end -- no rings for this

	if defenses[unitID] ~= nil then
		return
	end -- already has rings
...
function widget:FeatureCreated(featureID, allyTeam)
	...
	if featureDefID and featureDefIDtoUnitDefID[featureDefID] then
		...
		if unitID then
			if defenses[unitID] and defenses[unitID].allied == false and featureDefIDtoUnitDefID[featureDefID] == defenses[unitID].unitDefID then
...
if autoReload then
	function widget:DrawScreen()
		if defenseRangeShader.DrawPrintf then
			defenseRangeShader.DrawPrintf()
		end
	end
end
