...
local function base64(data)
	...
	for i = 1, n - 2, 3 do
		...
		oi = oi + 1
		out[oi] = B64:sub(floor(v / 262144) + 1, floor(v / 262144) + 1) .. B64:sub(floor(v / 4096) % 64 + 1, floor(v / 4096) % 64 + 1) .. B64:sub(floor(v / 64) % 64 + 1, floor(v / 64) % 64 + 1) .. B64:sub(v % 64 + 1, v % 64 + 1)
	end
	local tail = n % 3
	if tail == 1 then
		local a = data:byte(n)
	...
	elseif tail == 2 then
		...
		local v = a * 1024 + b * 4
		oi = oi + 1
		out[oi] = B64:sub(floor(v / 4096) + 1, floor(v / 4096) + 1) .. B64:sub(floor(v / 64) % 64 + 1, floor(v / 64) % 64 + 1) .. B64:sub(v % 64 + 1, v % 64 + 1) .. "="
	end
...
local status = { active = false, phase = "", step = 0, total = 0, message = "", lastPath = "", lastSummary = "", error = "" }
...
local function saveTargetRegion(tex, w, texH, imgY0, imgY1, path)
	...
	return false, (not okCall) and tostring(callErr) or (not ran) and "render target could not be bound" or "the image encoder refused it"
end
...
local function svgHeader(w, h, title)
	return format('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"\n' .. '     width="%d" height="%d" viewBox="0 0 %d %d" id="%s">\n', w, h, w, h, xmlEscape(title))
end
...
local function svgLabel(id, x, y, sizePx, text)
	return format('  <text id="%s" x="%.2f" y="%.2f" font-family="Arial" font-size="%.1f" ' .. 'fill="#ffffff" stroke="#000000" stroke-width="%.1f" paint-order="stroke" ' .. 'text-anchor="middle">%s</text>\n', xmlEscape(id), x, y, sizePx, max(1, sizePx * 0.18), xmlEscape(text))
end
...
local function phasePrep(j)
	...
	if j.mosaic then
		...
		j.stripMode = true
		j.stripH = j.maxRowH
		Echo(format("[TF Capture] %dx%d in one piece needs %d MB; writing one %dx%d strip per tile row instead", j.outW, j.outH, floor(j.outW * j.outH * 4 / 1048576), j.outW, j.stripH))
...
local function phaseTiles(j)
	...
	if not fits then
		...
		if j.fitTries > FIT_MAX_TRIES then
			...
			return finish(format("tile %d never fit on screen -- the camera cannot get high enough; try a higher detail setting (smaller tiles)", j.tileIndex))
		end
...
local function phaseSave(j)
	...
	if not j.stripMode then
		...
		if ok then
			...
			Echo(format("[TF Capture] one-piece save of %dx%d failed (%s); writing bands instead", j.outW, j.outH, tostring(err)))
...
local function phaseSprites(j)
	...
	setStatus("sprites", j.spriteIndex - 1, #j.spriteList, format("sprite %d / %d", min(j.spriteIndex - 1, #j.spriteList), #j.spriteList))
...
local function phaseSvg(j)
			...
			for i = 1, #group do
				...
				if uri then
					...
					if settings.unitLabels then
						out[#out + 1] = svgLabel(label .. " (label)", cx, cy + w * 0.5 + w * 0.22, max(8, w * 0.2), u.def.humanName or u.def.name)
					end
				end
			end
			out[#out + 1] = " </g>\n"
	...
	if settings.featureLayer and j.features and #j.features > 0 then
		...
		for i = 1, #j.features do
			...
			if uri then
				...
				out[#out + 1] = svgImage(format("%s - %s", f.def.humanName or f.def.name, f.def.name), cx - w * 0.5, cy - w * 0.5, w, w, 180 + (f.rot or 0) * 360 / 65536, uri)
			end
		end
		out[#out + 1] = " </g>\n</svg>\n"
	...
	if settings.commentLayer and WG.MapLabels and WG.MapLabels.getLabels then
		...
		if #labels > 0 then
			...
			for i = 1, #labels do
				...
				out[#out + 1] = format('  <circle id="pin %d" cx="%.2f" cy="%.2f" r="%.2f" fill="%s" stroke="#000000" stroke-width="%.2f"/>\n', L.id or i, cx, cy, r, xmlEscape(L.color or "#ffcc00"), max(1, r * 0.15))
	...
	Echo(format("[TF Capture] wrote %s (%dx%d in %d image%s, %d tiles, %d units, %d sprites)", j.dir, j.outW, j.outH, #j.terrainFiles, (#j.terrainFiles == 1) and "" or "s", #j.tiles, unitCount, #j.spriteList))
	if #j.terrainFiles > 1 then
...
local function startCapture()
	...
	if est.capped then
		Echo(format("[TF Capture] %.2fx is wider than the %d px texture limit -- capped to %.2fx (%dx%d)", est.detail, est.maxTex, est.effectiveDetail, est.outW, est.outH))
	end
	return true
end
