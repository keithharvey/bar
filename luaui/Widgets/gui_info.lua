...
local function refreshUnitInfo()
	...
	for unitDefID, unitDef in pairs(UnitDefs) do
		...
		if unitDef.name == "armdl" or unitDef.name == "cordl" or unitDef.name == "armlance" or unitDef.name == "cortitan" or (unitDef.minWaterDepth > 0 or unitDef.modCategories["ship"]) then
		...
		local function refreshUnitWeaponInfo(i, weaponDef, isPrimaryWeapon)
			...
			elseif weaponDef.shieldRadius and weaponDef.shieldRadius > 0 then
				...
				if unitDefInfo[unitDefID].mainWeapon == 0 then
					...
					unitDefInfo[unitDefID].reloadTime = weaponDef.customParams.dronesuesestockpile and weaponDef.stockpileTime or weaponDef.reload
				end
...
function GetColor(colormap, slider)
	...
	return col1[1] * ia + col2[1] * aa, col1[2] * ia + col2[2] * aa, col1[3] * ia + col2[3] * aa, col1[4] * ia + col2[4] * aa
end
...
local function drawSelectionCell(cellID, uDefID, usedZoom, highlightColor)
	...
	glColor(1, 1, 1, 1)
	UiUnit(cellRect[cellID][1] + cellPadding, cellRect[cellID][2] + cellPadding, cellRect[cellID][3], cellRect[cellID][4], cornerSize, 1, 1, 1, 1, usedZoom, nil, nil, unitTexture, nil, groups[unitGroup[uDefID]])
	tracy.ZoneEnd()
...
local function drawUnitInfo()
	...
	if unitDefInfo[displayUnitDefID].buildPic then
		...
		-- unit icon
		glColor(1, 1, 1, 1)
		UiUnit(iconX, iconY - iconSize, iconX + iconSize, iconY, nil, 1, 1, 1, 1, 0.03, nil, nil, "#" .. displayUnitDefID, (unitDefInfo[displayUnitDefID].icontype and ":l:" .. unitDefInfo[displayUnitDefID].icontype or nil), groups[unitGroup[displayUnitDefID]], { unitDefInfo[displayUnitDefID].metalCost, unitDefInfo[displayUnitDefID].energyCost })
		tracy.ZoneEnd()
		...
		for row = 1, rows do
			for coll = 1, colls do
				if unitDefInfo[displayUnitDefID].buildOptions[cellID] then
					...
					UiUnit(cellRect[cellID][1] + cellPadding, cellRect[cellID][2] + cellPadding, cellRect[cellID][3], cellRect[cellID][4], cellPadding * 1.3, 1, 1, 1, 1, 0.1, nil, disabled and 0 or nil, "#" .. uDefID, (unitDefInfo[uDefID].icontype and ":l:" .. unitDefInfo[uDefID].icontype or nil), groups[unitGroup[uDefID]], { unitDefInfo[uDefID].metalCost, unitDefInfo[uDefID].energyCost })
				end
				cellID = cellID - 1
				if cellID <= 0 then
					break
			...
			for row = 1, rows do
				for coll = 1, colls do
					if units[cellID] then
						...
						cellRect[cellID] = { math_floor(customInfoArea[3] - cellPadding - (coll * cellsize)), math_floor(customInfoArea[2] + cellPadding + ((row - 1) * cellsize)), math_floor(customInfoArea[3] - cellPadding - ((coll - 1) * cellsize)), math_floor(customInfoArea[2] + cellPadding + (row * cellsize)) }
						UiUnit(cellRect[cellID][1] + cellPadding, cellRect[cellID][2] + cellPadding, cellRect[cellID][3], cellRect[cellID][4], cellPadding * 1.3, 1, 1, 1, 1, 0.1, nil, nil, "#" .. uDefID, (unitDefInfo[uDefID].icontype and ":l:" .. unitDefInfo[uDefID].icontype or nil), groups[unitGroup[uDefID]], { unitDefInfo[uDefID].metalCost, unitDefInfo[uDefID].energyCost })
					end
					cellID = cellID - 1
					if cellID <= 0 then
						break
		...
		if displayMode == "selection" and selectionCells and selectionCells[1] and cellRect then
			for cellID, unitDefID in pairs(selectionCells) do
				if cellRect[cellID] and math_isInRect(x, y, cellRect[cellID][1], cellRect[cellID][2], cellRect[cellID][3], cellRect[cellID][4]) then
					...
					glBlending(GL_SRC_ALPHA, GL_ONE)
					RectRoundCircle(cellRect[cellID][1] + cellPadding + halfSize, 0, cellRect[cellID][2] + cellPadding + halfSize, halfSize, cornerSize, halfSize - math_max(1, cellPadding), { 1, 1, 1, 0.07 }, { 1, 1, 1, 0.07 })
					glBlending(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
