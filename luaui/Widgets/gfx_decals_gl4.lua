...
local function PackSavedDecals(savedDecals)
	...
	for i = 1, #savedDecals do
		...
		if entry and #entry == savedDecalFieldCount then
			packed[#packed + 1] = stringFormat("%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.6g,%d,%d,%d", entry[1], entry[2], entry[3], entry[4], entry[5], entry[6], entry[7], entry[8], entry[9], entry[10], entry[11], entry[12], entry[13])
		end
	end
...
function widget:Initialize()
	...
	if autoupdate then
		...
		for i = 1, 100 do
			...
			AddDecal(
				texture,
				Game.mapSizeX * mathRandom() * 1.0, --posx
				Game.mapSizeZ * mathRandom() * 1.0, --posz
				mathRandom() * 6.28, -- rotation
				w, -- width
				w, --height
				mathRandom() * 10000, -- heatstart
				mathRandom() * 1, -- heatdecay
				mathRandom() * 1.0 + 1.0, -- alphastart
				mathRandom() * 0.001, -- alphadecay
				mathRandom() * 0.3 + 0.7 -- maxalpha
			)
		end
	end
