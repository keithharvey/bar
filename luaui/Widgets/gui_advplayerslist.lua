...
--[[Changelog
    ...
    v47   (Attean): introduce a policy cache and TeamTransfer module, removing a lot of economic and unit specific code from this file and enabling unit_sharing_mode modoptions
]]--
...
local rankPics = {
    [0] = pics["rank0"], [1] = pics["rank1"], [2] = pics["rank2"], [3] = pics["rank3"],
    [4] = pics["rank4"], [5] = pics["rank5"], [6] = pics["rank6"], [7] = pics["rank7"],
}
...
-- Persists the last known player name per teamID across GetAllPlayers rebuilds
local lastKnownTeamNames = {}
local pendingInitRefresh = false  -- triggers one doPlayerUpdate after all widgets have initialized
...
local drawList = {}
local teamN
local prevClickTime = osClock()

-- Cached font scale factors (only depend on vsy, updated in ViewResize)
local fontScaleHigh = 1   -- math.clamp(1+((1-(vsy/1200))*0.75), 1, 1.25)
local fontScaleMed = 1    -- math.clamp(1+((1-(vsy/1200))*0.5), 1, 1.2)
local fontScaleLow = 1    -- math.clamp(1+((1-(vsy/1200))*0.4), 1, 1.15)
local fontScaleSpec = 1   -- math.clamp(1+((1-(vsy/1200))*0.66), 1, 1.33)
...
local function UpdateAlliances()
    ...
    -- Pre-filter to only active non-spec players with valid teams
    local activePlayers = {}
    for _, playerID in ipairs(playerList) do
        local p = player[playerID]
        if p and not p.spec and p.team ~= nil then
            activePlayers[#activePlayers + 1] = playerID
        end
    end
    -- Find cross-allyteam allies with cached lookups
    for i = 1, #activePlayers do
        ...
        local pTeam = p.team
        local alliances = {}
        for j = 1, #activePlayers do
            if i ~= j then
                local p2id = activePlayers[j]
                local p2 = player[p2id]
                if pAllyteam ~= p2.allyteam and sp.AreTeamsAllied(p2.team, pTeam) then
                    alliances[#alliances + 1] = p2id
                end
            end
        end
...
function widget:SelectionChanged(selectedUnits)
    -- Just record the selection; the per-receiver partition is computed lazily and
    -- memoised when a row actually needs it (draw/hover/share), not eagerly per player.
...
function widget:SharePolicyChanged(teamID, domain)
	...
	if domain == "unit" and teamID == myTeamID then
...
function widget:Initialize()
	...
	GetAliveAllyTeams()
	SortList()
	pendingInitRefresh = true  -- refresh after other widgets (e.g. playernames) have initialized
    SetModulesPositionX()
...
function GetAllPlayers()
    ...
    for i = 0, teamN - 1 do
        ...
        -- If team is empty, try to find who was on it before rebuilding the ghost
        if #teamPlayers == 0 and not lastKnownTeamNames[i] then
            ...
            -- Try via team leader stored in GetTeamInfo (works when player is now a spec)
            ...
            -- Try GetPlayerList including inactive/spec players
            if not lastKnownTeamNames[i] then
        ...
        if lastKnownTeamNames[i] then
            player[i + specOffset].lastKnownName = lastKnownTeamNames[i]
        end
...
function GetSkill(playerID)
    ...
    if osMu then
        ...
        if string.find(osMu, ")", nil, true) then
            osSkill = "\255" .. strChar(190) .. strChar(140) .. strChar(140) .. osSkill -- ')' means inferred from lobby rank
        else
            -- show privacy mode
            local priv = ""
            if string.find(osMu, "~", nil, true) then
            ...
            if not osSigma or tonumber(osSigma) > 6.65 then
                osSkill = priv .. "\255" .. strChar(tsRed) .. strChar(tsGreen) .. strChar(tsBlue) .. "??"
            else
                osSkill = priv .. "\255" .. strChar(tsRed) .. strChar(tsGreen) .. strChar(tsBlue) .. osSkill
            end

        end
    else
        osSkill = "\255" .. strChar(160) .. strChar(160) .. strChar(160) .. "?"
    end
    return osSkill
end
...
function CreatePlayer(playerID)
	...
	if accountID then
        local existingID = accountIDLookup[accountID]
        if existingID ~= nil and existingID ~= playerID then
            accountID = nil
        else
            accountIDLookup[accountID] = playerID
        end
	end
...
function CreatePlayerFromTeam(teamID)
    ...
    if isAI then
        ...
        if Spring.GetGameSeconds() < 0.1 then
            ...
            tdead = isDead  -- winning players who leave should not show as dead
        end
...
function UpdatePlayerResources()
    ...
    for playerID, _ in pairs(player) do
        local p = player[playerID]
        if (playerID < specOffset or p.ai) and p.name and not p.spec and p.team then        -- maybe not filter out specs cause resigned players can still hold storage
			if aliveAllyTeams[p.allyteam] ~= nil and (mySpecStatus or myAllyTeamID == p.allyteam) then
				if (mySpecStatus and enemyListShow) or p.allyteam == myAllyTeamID then	-- only keep track when its being displayed
                    displayedPlayers = displayedPlayers + 1
					energy, energyStorage, _, energyIncome, _, energyShare = sp.GetTeamResources(p.team, "energy")
					metal, metalStorage, _, metalIncome, _, metalShare = sp.GetTeamResources(p.team, "metal")
					if energy == nil then
					...
					p.energy = energy
					p.energyIncome = energyIncome
					p.energyStorage = energyStorage
					p.energyShare = energyShare
					p.metal = metal
					p.metalIncome = metalIncome
					...
					if not allyTeamMaxStorage[pAllyteam] then
						allyTeamMaxStorage[pAllyteam] = {}
					end
					local ats = allyTeamMaxStorage[pAllyteam]
					if not ats[1] or energyStorage > ats[1] then
						ats[1] = energyStorage
					end
					if not ats[2] or metalStorage > ats[2] then
						ats[2] = metalStorage
					end
				end
			end
...
function SortSpecs(vOffset)
    ...
    -- Single pass: collect active specs and count them
    local activeSpecs = {}
    for _, playerID in ipairs(playersList) do
        local _, active, spec = sp.GetPlayerInfo(playerID, false)
        if spec and active then
            activeSpecs[#activeSpecs + 1] = playerID
        end
    end
    ...
    local noSpec = true
    for _, playerID in ipairs(activeSpecs) do
...
function widget:DrawScreen()
	...
	if mainListBgTex then
		gl.R2tHelper.BlendTexRect(mainListBgTex, apiAbsPosition[2], apiAbsPosition[3], apiAbsPosition[4], apiAbsPosition[1], true)
	end

	if mainListTex then
		gl.R2tHelper.BlendTexRect(mainListTex, apiAbsPosition[2], apiAbsPosition[3], apiAbsPosition[4], apiAbsPosition[1], true)
	end
	if mainList2Tex then
		gl.R2tHelper.BlendTexRect(mainList2Tex, apiAbsPosition[2], apiAbsPosition[3], apiAbsPosition[4], apiAbsPosition[1], true)
	end
    ...
    if m_point.active then
        ...
        for i, drawObject in ipairs(drawList) do
            if drawObject >= 0 then
                ...
                if p.pointTime ~= nil or p.pencilTime ~= nil or p.eraserTime ~= nil then
                    local posY = widgetPosY + widgetHeight - drawListOffset[i]
                    local isAllyOrSpec = p.allyteam == myAllyTeamID or mySpecStatus
                    if isAllyOrSpec then
                        if p.pointTime ~= nil then
                            DrawPoint(posY, p.pointTime - now)
                        end
                        if p.pencilTime ~= nil then
                            DrawPencil(posY, p.pencilTime - now)
                        end
                        if p.eraserTime ~= nil then
    ...
    for _, i in ipairs(drawList) do
        if i > -1 then -- and i < specOffset
            local p = player[i]
            posY = widgetPosY + widgetHeight - (p.posY or 0)
            if myTeamID ~= p.team and not p.spec and not p.dead and p.name ~= absentName and IsOnRect(x, y, widgetPosX, posY, widgetPosX + widgetWidth, posY + (playerOffset*playerScale)) then
                if mySpecStatus or (myAllyTeamID == p.allyteam and not sliderPosition) then
...
function doCreateLists(onlyMainList, onlyMainList2, onlyMainList3)
    ...
    if onlyMainList2 then
        if tipTextTime+(updateFastRate*updateFastRateMult) < osClock() then
            tipText = nil
            tipTextTitle = nil
            drawTipText = nil
            tipTextTime = 0
...
function CheckTime()
    ...
    if now > (lastTime + period) then
        ...
        CheckPlayersChange()
        blink = not blink
        for playerID in pairs(activeDrawPlayers) do
            local p = player[playerID]
            if p then
                local stillActive = false
                if p.pointTime ~= nil then
                    if p.pointTime <= now then
                        p.pointX = nil
                        p.pointY = nil
                        p.pointZ = nil
                        p.pointTime = nil
                ...
                if not stillActive then
                    activeDrawPlayers[playerID] = nil
                end
            else
                activeDrawPlayers[playerID] = nil
            end
...
function DrawPlayer(playerID, leader, vOffset, mouseX, mouseY, onlyMainList, onlyMainList2, onlyMainList3)
    ...
    local name = p.name
	local nameIsAlias = p.nameIsAlias
    local team = p.team
    if not team then return end -- this prevents error when co-op / joinas is active

    local allyteam = p.allyteam
    local pingLvl = p.pingLvl
    local cpuLvl = p.cpuLvl
    local ping = p.ping
    local cpu = p.cpu
    local spec = p.spec
    ...
    if playerID >= specOffset and not ai then
        posY = posY - (deadPlayerHeightReduction / 2) * playerScale
    end
    local tipPosY = widgetPosY + ((widgetHeight - vOffset) * widgetScale)
	local desynced = p.desynced
	local accountID = p.accountID
    ...
    if not spec then
        ...
        if (onlyMainList2 or onlyMainList3) and not isSingle and (ModuleRefs.resources.active or ModuleRefs.income.active) and aliveAllyTeams[allyteam] ~= nil and p.energy ~= nil then
            if (mySpecStatus and enemyListShow) or myAllyTeamID == allyteam then
                ...
                local ei = p.energyIncome
                local esh = p.energyShare
                local ec = p.energyConversion
                local m = p.metal
                local ms = p.metalStorage
...
function DrawResources(energy, energyStorage, energyShare, energyConversion, metal, metalStorage, metalShare, posY, dead, maxAllyTeamEnergyStorage, maxAllyTeamMetalStorage)
    ...
    if metalShare < 0.99 then  -- default = 0.99
        gl_Color(0,0,0, 0.18)
        gl_Texture(false)
        DrawRect(ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (metalStorage/maxStorage)) * metalShare) - 0.75 - bordersize,
                posY + y1Offset + 0.55 + bordersize,
                ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (metalStorage/maxStorage)) * metalShare) + 0.75 + bordersize,
                posY + y2Offset - 0.55 - bordersize)
        gl_Color(1, 0.25, 0.25, 1)
        gl_Texture(pics["resbarPic"])
        DrawRect(ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (metalStorage/maxStorage)) * metalShare) - 0.75,
                posY + y1Offset + 0.55,
                ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (metalStorage/maxStorage)) * metalShare) + 0.75,
                posY + y2Offset - 0.55)
    end
    ...
    if energyConversion and energyConversion ~= 0.75 and not dead then    -- default = 0.75
        ...
        gl_Texture(false)
        DrawRect(ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyConversion) - 0.75 - bordersize,
                posY + y1Offset + 0.55 + bordersize,
                ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyConversion) + 0.75 + bordersize,
                posY + y2Offset - 0.55 - bordersize)
        gl_Color(0.9, 0.9, 0.73, 1)
        gl_Texture(pics["resbarPic"])
        DrawRect(ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyConversion) - 0.75,
                posY + y1Offset + 0.55,
                ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyConversion) + 0.75,
                posY + y2Offset - 0.55)
    end
    ...
    if energyShare < 0.94 or energyShare > 0.96 then  -- default = 0.94999999
        gl_Color(0,0,0, 0.18)
        gl_Texture(false)
        DrawRect(ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyShare) - 0.75 - bordersize,
                posY + y1Offset + 1.1,
                ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyShare) + 0.75 + bordersize,
                posY + y2Offset - 1.1)
        gl_Color(1, 0.25, 0.25, 1)
        gl_Texture(pics["resbarPic"])
        DrawRect(ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyShare) - 0.75,
                posY + y1Offset + 0.55,
                ModuleRefs.resources.posX + widgetPosX + paddingLeft + ((barWidth * (energyStorage/maxStorage)) * energyShare) + 0.75,
                posY + y2Offset - 0.55)
    end
end

function DrawIncome(energy, metal, posY, dead)
    local fontsize = (dead and 4.5 or 8.5) * fontScaleLow
    ...
    if energy > 0 then
        font:Print(
                ...
                ModuleRefs.income.posX + widgetPosX + ModuleRefs.income.width - 2,
...
function DrawRank(rank, posY)
    local rankPic = rankPics[rank]
    if rankPic then
        DrawRankImage(rankPic, posY)
    end
end
...
function DrawCountry(country, posY)
    if country == nil or country == "??" then return end
    ...
    if cached == nil then
        ...
        countryFlagCache[country] = cached
    end
    if cached then
        gl_Texture(cached)
        gl_Color(1, 1, 1, 1)
...
function DrawAlliances(alliances, posY)
    ...
    for i, allyPlayerID in pairs(alliances) do
        local ap = player[allyPlayerID]
        if ap ~= nil and ap.red ~= nil then
            gl_Color(0, 0, 0, 0.25)
            RectRound(posX + (width * (i - 1)), posY - (3*playerScale), posX + (width * i), posY + (19*playerScale), (2*playerScale))
            gl_Color(ap.red, ap.green, ap.blue, 0.5)
...
function DrawName(name, nameIsAlias, team, posY, dark, playerID, accountID, desynced)
    ...
    local pDraw = player[playerID]
    local isAbsent = false
    if name == absentName then
        isAbsent = true
        local lastKnownName = pDraw and pDraw.lastKnownName
        if not lastKnownName and team then
            ...
            if teamLeaderID and teamLeaderID >= 0 then
                ...
                if pName and pName ~= "" and not pSpec and pTeam == team then
                    ...
                    lastKnownTeamNames[team] = lastKnownName
                    if pDraw then pDraw.lastKnownName = lastKnownName end
                end
            end
        end
        if lastKnownName then
            name = lastKnownName
        end
    end
    ...
    local nameText = name
	if WG.playernames and not pDraw.history then
		pDraw.history = WG.playernames.getAccountHistory(accountID) or {}
	end
	if nameIsAlias then
		nameText = "." .. nameText
	elseif pDraw.history and #pDraw.history > 1 then
		nameText = "*" .. nameText
	else
		nameText = " " .. nameText
	end
    ...
    font2:Begin(true)
    local fontsize = (isAbsent and 9 or 14) * fontScaleMed
...
function DrawSmallName(name, nameIsAlias, team, posY, dark, playerID, accountID, alpha)
    ...
    local nameText = name
	local pSmall = player[playerID]
	if WG.playernames and not pSmall.history then
		pSmall.history = WG.playernames.getAccountHistory(accountID) or {}
	end
	if nameIsAlias then
		nameText = "." .. name
	elseif pSmall.history and #pSmall.history > 1 then
		nameText = "*" .. name
    ...
    if originalColourNames[playerID] then
        local oc = originalColourNames[playerID]
        nameText = "\255" .. strChar(oc[1]) .. strChar(oc[2]) .. strChar(oc[3]) .. nameText
    end
...
function DrawPlayerID(playerID, posY, dark, spec)
    ...
    local fontsize = 9.5 * (usedScale + ((1-usedScale)*0.25)) * fontScaleHigh
...
function CheckPlayersChange()
    local sorting = false
    local curGameFrame = spGetGameFrame()
        ...
        if active == false then
            if p.name ~= nil then
                ...
                if p.spec == false then
                    if table.maxn(sp.GetPlayerList(p.team, true)) == 0 then
                        player[p.team + specOffset] = CreatePlayerFromTeam(p.team)
                        player[p.team + specOffset].lastKnownName = p.name
                        sorting = true
                    end
                end
            end
        elseif active and name ~= nil then
            if spec ~= p.spec then
                ...
                if spec then
                    ...
                    p.team = nil -- remove team
                end
                p.spec = spec -- consider player as spec
                sorting = true
            end
            if teamID ~= p.team then
                ...
                p.team = teamID
				if (not mySpecStatus) and anonymousMode ~= "disabled" and teamID ~= myTeamID then
					p.red, p.green, p.blue = anonymousTeamColor[1], anonymousTeamColor[2], anonymousTeamColor[3]
				else
					p.red, p.green, p.blue = sp.GetTeamColor(teamID)
				end
                p.dark = Color.IsDark(p.red, p.green, p.blue)
                p.skill = GetSkill(i)
                sorting = true
            end
            if p.name == nil then
                player[i] = CreatePlayer(i)
                p = player[i]  -- refresh after reassignment
                if p.name ~= nil then
                    forceMainListRefresh = true
                end
            end
            if allyTeamID ~= p.allyteam then
                p.allyteam = allyTeamID
                updateTake(allyTeamID)
                sorting = true
            end
            if desynced ~= p.desynced then
                forceMainListRefresh = true
                sorting = true
            end
            ...
            p.cpuLvl = GetCpuLvl(cpuUsage)
            p.ping = pingTime * 1000 - ((pingTime * 1000) % 1)
            p.cpu = cpuUsage * 100 - ((cpuUsage * 100) % 1)
			p.desynced = desynced
        end

        if teamID and curGameFrame > 0 then
...
function widget:Update(delta)
    ...
    if clickedPlayerTime and osClock() - clickedPlayerTime > dblclickPeriod then
...
function widget:ViewResize()
    ...
    -- Update cached font scale factors
    local vsyRatio = 1 - (vsy / 1200)
    fontScaleHigh = math.clamp(1 + (vsyRatio * 0.75), 1, 1.25)
    fontScaleMed  = math.clamp(1 + (vsyRatio * 0.5), 1, 1.2)
    fontScaleLow  = math.clamp(1 + (vsyRatio * 0.4), 1, 1.15)
    fontScaleSpec = math.clamp(1 + (vsyRatio * 0.66), 1, 1.33)
...
function widget:MapDrawCmd(playerID, cmdType, px, py, pz)
    ...
    if m_point.active then
        if cmdType == "point" then
            ...
            activeDrawPlayers[playerID] = true
