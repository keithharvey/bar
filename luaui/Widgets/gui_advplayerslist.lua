		...
		if #teamPlayers == 0 and not lastKnownTeamNames[i] then
			...
			if not lastKnownTeamNames[i] then
				...
				if teamLeaderID and teamLeaderID >= 0 then
					...
					if pName and not pSpec and pTeam == i then
						lastKnownTeamNames[i] = (WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(teamLeaderID) or pName
					end
				end
			end
			...
			if not lastKnownTeamNames[i] then
				...
				for _, pID in ipairs(allTeamPlayers) do
					...
					if pName and not pSpec and pTeam == i then
						lastKnownTeamNames[i] = (WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(pID) or pName
						break
					end
				end
			end
...
function CreateLists(onlyMainList, onlyMainList2, onlyMainList3)
	...
	if onlyMainList2 == nil then
		onlyMainList2 = true
	end
	if onlyMainList3 == nil then
		onlyMainList3 = true
	end
...
function DrawSeparator(vOffset)
	...
	RectRound(widgetPosX + 2, widgetPosY + widgetHeight - vOffset - (1.5 / widgetScale), widgetPosX + widgetWidth - 2, widgetPosY + widgetHeight - vOffset + (1.5 / widgetScale), (0.5 / widgetScale), 1, 1, 1, 1, { 0.66, 0.66, 0.66, alpha }, { 0, 0, 0, alpha })
end
...
function DrawPlayer(playerID, leader, vOffset, mouseX, mouseY, onlyMainList, onlyMainList2, onlyMainList3)
	...
	if not team then
		return
	end -- this prevents error when co-op / joinas is active
...
function DrawResources(energy, energyStorage, energyShare, energyConversion, metal, metalStorage, metalShare, posY, dead, maxAllyTeamEnergyStorage, maxAllyTeamMetalStorage)
	...
	if not maxStorage or maxStorage == 0 then
		return
		...
		if not lastKnownName and team then
			...
			if teamLeaderID and teamLeaderID >= 0 then
				...
				if pName and pName ~= "" and not pSpec and pTeam == team then
					lastKnownName = (WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(teamLeaderID) or pName
					-- Persist so GetAllPlayers picks it up and doesn't look it up again
					lastKnownTeamNames[team] = lastKnownName
					if pDraw then
						pDraw.lastKnownName = lastKnownName
					end
				end
			end
		end
...
function GetCpuLvl(cpuUsage)
	if cpuUsage < pingLevelData[1].cpuThreshold then
		return 1
	elseif cpuUsage < pingLevelData[2].cpuThreshold then
		return 2
	elseif cpuUsage < pingLevelData[3].cpuThreshold then
		return 3
	elseif cpuUsage < pingLevelData[4].cpuThreshold then
		return 4
	else
		return 5
	end
...
function GetPingLvl(ping)
	if ping < pingLevelData[1].pingThreshold then
		return 1
	elseif ping < pingLevelData[2].pingThreshold then
		return 2
	elseif ping < pingLevelData[3].pingThreshold then
		return 3
	elseif ping < pingLevelData[4].pingThreshold then
		return 4
	else
		return 5
	end
...
function widget:Update(delta)
	...
	if m_take.active and mySpecStatus == false and mx >= widgetPosX - 57 * widgetScale and mx <= widgetPosX - 1 * widgetScale then
		for i, drawObject in ipairs(drawList) do
			if drawObject >= 0 then
				local p = player[drawObject]
				if p and not p.spec and p.allyteam == myAllyTeamID and p.totake then
