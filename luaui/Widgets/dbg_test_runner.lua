...
Test = {
	...
	prepareWidget = function(widgetName)
		...
		assert(
			widgetHandler.knownWidgets[widgetName] ~= nil,
...
local function step()
	...
	if coroutine.status(activeTestState.coroutine) == "suspended" then
		...
		resumeOk, resumeResult =
			coroutine.resume(activeTestState.coroutine, coroutineOk, coroutineArgs and unpack(coroutineArgs) or nil)
