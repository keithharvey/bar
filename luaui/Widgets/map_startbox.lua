...
local function shouldRenderTeam(teamID, excludeMyTeam)
	...
	local isVisible = (not spec or isAI) and teamID ~= gaiaTeamID and (not isAI or teamAllyTeamID == myAllyTeamID or isSpec or allowEnemyAIPlacement)
		...
		if updateCounter % 30 == 0 then
			...
			if not hasChanges then
				for teamID, placement in pairs(currentPlacements) do
					if not lastKnownPlacements[teamID] or lastKnownPlacements[teamID].x ~= placement.x or lastKnownPlacements[teamID].z ~= placement.z then
						hasChanges = true
						break
					end
				end
