...
local function refreshCommands()
	...
	for _, command in ipairs(activeCmdDescs) do
		if type(command) == "table" and not disabledCommand[command.name] then
			...
			if not hiddenCommands[command.id] and not hiddenCommandTypes[command.type] and command.action ~= nil and not command.disabled and not (command.id == CANCEL_TARGET_CMD_ID and not cancelTargetRelevant) then
...
local function drawHighlights()
	...
	for cell = 1, #commands do
		...
		if hl and rect and rect[4] then
			...
			WG.FlowUI.Draw.RectRoundOutline(x1, y1, x2, y2, cs, thickness, 1, 1, 1, 1, { r, g, b, locOutlineAlpha }, { r, g, b, locOutlineAlpha * 0.85 })

			-- Soft inner glow fading inward
			local glowWidth = thickness * 3
			WG.FlowUI.Draw.RectRoundOutline(x1 + thickness, y1 + thickness, x2 - thickness, y2 - thickness, math_max(0, cs - thickness), glowWidth, 1, 1, 1, 1, { r, g, b, locGlowAlpha }, { r, g, b, 0 })
...
local function drawStateLights(cell, leftMargin, rightMargin, bottomMargin, padding, cellInnerWidth, cellInnerHeight, statecount, fillMin, fillMax, desiredState)
	...
	for i = 1, statecount do
		...
		if rows < 6 then
			RectRound(x1, y1, x2, y2, stateHeight * 0.33, (i == 1 and 0 or 2), (i == statecount and 0 or 2), (i == statecount and 2 or 0), (i == 1 and 2 or 0))
		else
			glRect(x1, y1, x2, y2)
		end
...
local function drawCell(cell, zoom)
	...
	if cmd then
		...
		if not showIcons or not cursorTextures[cmd.cursor] then
			...
			elseif colorize > 0 and commandInfo[cmd.action] and commandInfo[cmd.action].red then
				...
				if not colorStrCache[cmd.action] then
					...
					colorStrCache[cmd.action] = BAR.Utilities.ConvertColor((grey + info.red) / part, (grey + info.green) / part, (grey + info.blue) / part)
				end
		...
		if isStateCommand[cmd.id] or cmd.id == CMD.WAIT then
			...
			if cache and cache.statecount == statecount and cache.fillMin == fillMin and cache.fillMax == fillMax and cache.desiredState == desiredState then
