...
state.startMapmarkInput = function(x, y, z, triggerKey, triggerScanCode, waitForTriggerRelease)
	...
	state.mapmarkTriggerIdentityCaptured = triggerKey ~= nil or triggerScanCode ~= nil or (state.mapmarkPressedKeys and next(state.mapmarkPressedKeys) ~= nil) or (state.mapmarkPressedScans and next(state.mapmarkPressedScans) ~= nil)
...
state.mapActionMatchesCurrentModifiers = function(command, actions)
	...
	for i = 1, #actions do
		...
		if action.command == command and type(action.boundWith) == "string" then
			...
			if anyModifiers or (alt == expectedAlt and ctrl == expectedCtrl and meta == expectedMeta and shift == expectedShift) then
				return true
			end
		end
	end
...
function widget:TextInput(char) -- if it isnt working: chobby probably hijacked it
	if handleTextInput and not chobbyInterface and not Spring.IsGUIHidden() and showTextInput then
		if inputMode == "label" and (state.mapmarkTriggerDown or state.mapmarkTextInputPending or state.mapmarkAwaitingFreshKeyPress) then
			return true
		end
...
function widget:KeyRelease(key, mods, label, unicode, scanCode)
	...
	if inputMode == "label" and state.mapmarkTriggerDown and (not state.mapmarkTriggerIdentityCaptured or key == state.mapmarkTriggerKey or (scanCode and scanCode == state.mapmarkTriggerScanCode)) then
		state.finishMapmarkTriggerWait()
		return true
	end
	if state.mapDrawActive and (key == state.mapDrawKey or (scanCode and scanCode == state.mapDrawScanCode)) then
...
function widget:KeyPress(key, mods, isRepeat, label, unicode, scanCode, actions)
	...
	if inputMode == "label" and state.mapmarkAwaitingFreshKeyPress and not state.mapmarkTriggerDown and not state.mapmarkTextInputPending and not isRepeat then
		state.mapmarkAwaitingFreshKeyPress = false
	end
