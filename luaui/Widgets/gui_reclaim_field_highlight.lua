...
GetClusterVisibility = function(cid, isEnergy, currentDrawCount)
	...
	elseif cached and cached.lastInViewDraw and currentDrawCount - cached.lastInViewDraw <= animCfg.visibilityGraceDraws then
		inView = true
...
do
	...
	processCluster = function(cluster, clusterID, points, resourceType, targetHulls, targetClusters, nextClusterId, previousCluster, previousHull, alternateCluster, alternateHull)
		...
		local exactPreviousMembership = previousCluster and previousCluster.members and #previousCluster.members == #points
		...
		if previousCluster and previousHull and previousCluster.font ~= 0 and previousCluster.geometrySegments == geometrySegments and previousCluster.geometryTerrainGeneration == batch.terrainGeneration then
		...
		if not reusableCluster and alternateCluster and alternateHull and alternateCluster.font ~= 0 and alternateCluster.geometrySegments == geometrySegments and alternateCluster.geometryTerrainGeneration == batch.terrainGeneration and alternateCluster.members and #alternateCluster.members == #points then
	...
	local function Run(_self, targetClusters, targetHulls, alternateClusters, alternateHulls)
		...
		while rootIndex <= rootCandidateCount do
			...
			if unprocessed[featureID] == true then
				...
				local alternateCid = alternateCidField and point[alternateResourceType] >= minFeatureValue and point[alternateCidField]
				...
				while workCount > 0 do
					...
					if sameAlternateCluster and (pt[alternateResourceType] < minFeatureValue or pt[alternateCidField] ~= alternateCid) then
		...
		for cid = 1, clusterID do
			...
			local newClusters, reusedHull, copiedHull, partialReuse = processCluster(cluster, cid, cluster.members, currentResourceType, targetHulls, targetClusters, nextClusterId, previousCluster, previousHull, alternateCluster, alternateHull)
			if reusedHull then
				if currentResourceType == "energy" then
...
do
	...
	UpdateDrawEnabled = function()
		...
		if not previousDrawEnabled and drawEnabled then
			...
			batch.waitForFreshMetal = gameStarted and (batch.clusterJobActive or dirty.needCluster or batch.pendCreateCount > batch.pendCreateHead or batch.pendDestrCount > batch.pendDestrHead)
...
do
	...
	UpdateDrawEnergyEnabled = function()
		...
		if not previousDrawEnergyEnabled and drawEnergyEnabled then
			...
			batch.waitForFreshEnergy = gameStarted and (batch.clusterJobActive or dirty.needCluster or batch.pendCreateCount > batch.pendCreateHead or batch.pendDestrCount > batch.pendDestrHead)
...
local function UpdateReclaimFields()
	...
	if batch.clusterJobActive then
		...
		if debugTiming then
			...
			if dt * 1000 >= timingAccum.spikeMs and (now - timingAccum.lastSpikeClock) >= timingAccum.spikeMinGap then
				...
				Spring.Echo(string.format("[ReclaimField SPIKE] cluster-slice=%.2fms  features=%d  pendingCreate=%d/%d  pendingDestr=%d/%d", dt * 1000, cachedKnownFeaturesCount, batch.pendCreateCount - batch.pendCreateHead, batch.pendCreateCount, batch.pendDestrCount - batch.pendDestrHead, batch.pendDestrCount))
			end
		end
		if not ok then
			batch.finishClusterTracy()
		...
		elseif coroutine.status(batch.clusterJobCo) == "dead" then
			...
			if debugTiming then
				...
				Spring.Echo(string.format("[ReclaimField CLUSTER] cpu=%.2fms finalize=%.2fms clusters=%d/%d reused=%d/%d partial=%d/%d copiedE=%d", batch.lastClusterJobCpu * 1000, dt * 1000, #featureClusters, #energyFeatureClusters, batch.reusedMetalHulls, batch.reusedEnergyHulls, batch.partialReusedMetalHulls, batch.partialReusedEnergyHulls, batch.copiedEnergyHulls))
				if dt * 1000 >= timingAccum.spikeMs and (now - timingAccum.lastSpikeClock) >= timingAccum.spikeMinGap then
					timingAccum.lastSpikeClock = now
					Spring.Echo(string.format("[ReclaimField SPIKE] cluster-finalize=%.2fms  metalClusters=%d  energyClusters=%d  reused=%d/%d", dt * 1000, #featureClusters, #energyFeatureClusters, batch.reusedMetalHulls, batch.reusedEnergyHulls))
				end
			end
		end
		return
	...
	local refreshPending = dirty.needCluster or batch.pendCreateCount > batch.pendCreateHead or batch.pendDestrCount > batch.pendDestrHead
	...
	if dirty.needRedraw == true then
		...
		if debugTiming then
			...
			if dt * 1000 >= timingAccum.spikeMs and (now - timingAccum.lastSpikeClock) >= timingAccum.spikeMinGap then
				timingAccum.lastSpikeClock = now
				Spring.Echo(string.format("[ReclaimField SPIKE] redraw-lists=%.2fms  rebuildsThisFrame=%d  clusters=%d", dt * 1000, timingAccum.rebuilds, #featureClusters))
			end
		end
	end
...
function widget:Initialize()
	...
	WG.reclaimfieldhighlight.printTimingNow = function()
		...
		Spring.Echo(string.format("[ReclaimField TIMING NOW] avg(ms): UpdReclaim=%.3f DrawText=%.3f DrawPre=%.3f Update=%.3f DefPend=%.3f Poll=%.3f Slice=%.3f Final=%.3f Redraw=%.3f | max(ms): Upd=%.2f DrawPre=%.2f Slice=%.2f Final=%.2f Redraw=%.2f | DL/frame=%.1f lastJob=%.2fms reuse=%d/%d partial=%d/%d copiedE=%d", timingAccum.updateReclaim / denom * 1000, timingAccum.drawWorldText / denom * 1000, timingAccum.drawPreUnit / denom * 1000, timingAccum.updateFunc / denom * 1000, timingAccum.deferPending / denom * 1000, timingAccum.reclaimPoll / denom * 1000, timingAccum.clusterSlice / denom * 1000, timingAccum.clusterFinalize / denom * 1000, timingAccum.redrawLists / denom * 1000, timingAccum.maxUpdateReclaim * 1000, timingAccum.maxDrawPreUnit * 1000, timingAccum.maxClusterSlice * 1000, timingAccum.maxClusterFinalize * 1000, timingAccum.maxRedrawLists * 1000, timingAccum.rebuilds / denom, batch.lastClusterJobCpu * 1000, batch.reusedMetalHulls, batch.reusedEnergyHulls, batch.partialReusedMetalHulls, batch.partialReusedEnergyHulls, batch.copiedEnergyHulls))
	end
...
local function DrawFadingCluster(uid, entry, drawGradient)
	...
	if drawGradient then
		...
		local wantRebuild = mustRebuild or not entry.lastBakedAlpha or abs(alpha - entry.lastBakedAlpha) > animCfg.rebuildThreshold
		if wantRebuild then
			if animCfg.rebuildBudgetFrame ~= drawCounter then
				animCfg.rebuildBudgetFrame = drawCounter
				local cameraMoving = (drawCounter - animCfg.cameraMoveDraw) <= animCfg.cameraSettleDraws
...
function widget:DrawWorld()
	...
	local showEnergy = not batch.waitForFreshEnergy and ((showEnergyFields and drawEnergyEnabled and not allEnergyFieldsDrained) or (showEnergyFields and animState.toggleEnergy > 0.005) or hasFadingEnergy)
	...
	if widgetFont then
		if showMetal then
			...
			for clusterID = 1, #featureClusters do
				...
				if cluster and cluster.textX and clusterData and clusterData.gradient and clusterData.generation == batch.clusterGeneration then
	...
	if showResourceIcons then
		...
		if showMetal then
			...
			for clusterID = 1, #featureClusters do
				local cluster = featureClusters[clusterID]
				local clusterData = clusterDisplayLists[clusterID]
				if cluster and cluster.textX and cluster.text and clusterData and clusterData.gradient and clusterData.generation == batch.clusterGeneration then
					local effAlpha = animState.GetClusterAnimAlphaAndScale(cluster.uid, false)
					if effAlpha > 0.01 then
						local fs = cluster.font
...
function widget:DrawWorldPreUnit()
	...
	if shouldUpdateReclaim then
		...
		if debugTiming then
			...
			if dt * 1000 >= timingAccum.spikeMs and (osClock() - timingAccum.lastSpikeClock) >= timingAccum.spikeMinGap then
				...
				Spring.Echo(string.format("[ReclaimField SPIKE] UpdateReclaimFields=%.2fms  features=%d  metalClusters=%d  energyClusters=%d", dt * 1000, cachedKnownFeaturesCount, #featureClusters, #energyFeatureClusters))
			end
		end
	end
	...
	if debugTiming and timingCount >= timingInterval then
		...
		Spring.Echo(string.format("[ReclaimField TIMING] avg(ms): UpdReclaim=%.3f DrawText=%.3f DrawPre=%.3f Update=%.3f DefPend=%.3f Poll=%.3f Slice=%.3f Final=%.3f Redraw=%.3f | max(ms): Upd=%.2f DrawPre=%.2f Slice=%.2f Final=%.2f Redraw=%.2f | DL/frame=%.1f clusters=%d features=%d pendC=%d pendD=%d defC=%d defD=%d job=%s lastJob=%.2fms reuse=%d/%d partial=%d/%d copiedE=%d", timingAccum.updateReclaim / div * 1000, timingAccum.drawWorldText / div * 1000, timingAccum.drawPreUnit / div * 1000, timingAccum.updateFunc / div * 1000, timingAccum.deferPending / div * 1000, timingAccum.reclaimPoll / div * 1000, timingAccum.clusterSlice / div * 1000, timingAccum.clusterFinalize / div * 1000, timingAccum.redrawLists / div * 1000, timingAccum.maxUpdateReclaim * 1000, timingAccum.maxDrawPreUnit * 1000, timingAccum.maxClusterSlice * 1000, timingAccum.maxClusterFinalize * 1000, timingAccum.maxRedrawLists * 1000, timingAccum.rebuilds / div, #featureClusters, cachedKnownFeaturesCount, batch.pendCreateCount - batch.pendCreateHead, batch.pendDestrCount - batch.pendDestrHead, batch.deferCreateCount, batch.deferDestrCount, batch.clusterJobActive and "1" or "0", batch.lastClusterJobCpu * 1000, batch.reusedMetalHulls, batch.reusedEnergyHulls, batch.partialReusedMetalHulls, batch.partialReusedEnergyHulls, batch.copiedEnergyHulls))
