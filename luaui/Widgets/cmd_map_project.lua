...
local function stepFeatures()
	...
	for _, e in ipairs(entries) do
		...
		if e.pitch and e.roll and e.y then
			lines[#lines + 1] = format("		{ name = %q, x = %.1f, z = %.1f, rot = %d, pitch = %.4f, roll = %.4f, y = %.1f },", e.name, e.x, e.z, e.rot, e.pitch, e.roll, e.y)
		else
...
local function stepUnits()
	...
	local format = string.format
	for _, e in ipairs(entries) do
		lines[#lines + 1] = format("\t\t{ name = %q, x = %.1f, z = %.1f, rot = %d, team = %d%s },", e.name, e.x, e.z, e.rot, e.team, e.neutral and ", neutral = true" or "")
	end
	lines[#lines + 1] = "\t},"
	lines[#lines + 1] = "}"
...
local function deleteProject(slug)
	...
	if failed > 0 then
		echoP(string.format("deleted '%s' (%d files, %d could not be removed — folder may linger in %s)", slug, removed, failed, PROJECTS_DIR))
	else
...
local function phaseUnits(c)
	...
	if not c.sent then
		...
		for _, u in ipairs(data.units) do
			...
			if type(u.name) == "string" and x and z then
				count = count + 1
				parts[#parts + 1] = format("%s %.1f %.1f %d %d %d", u.name, x, z, tonumber(u.rot) or 0, tonumber(u.team) or 0, u.neutral and 1 or 0)
				if #parts >= 25 then
	...
	if ack > c.ackBase then
		...
		loadOk("units", spawned .. " spawned" .. (failed > 0 and (", " .. failed .. " FAILED (unknown def or unit limit)") or "") .. (remapped > 0 and (", " .. remapped .. " remapped to Gaia (team missing/dead)") or ""))
		return true
	end
	c.ticks = c.ticks + 1
	local frame = Spring.GetGameFrame()
...
local function maybeStartLoad()
	...
	local isBlankCanvas = type(mo) == "table" and ((tonumber(mo.blank_map_x) or 0) > 0 or (tonumber(mo.blank_map_y) or 0) > 0)
