...
local function syncGhosts(items)
	...
	if shown > PREVIEW_GHOST_CAP then
		...
		if not preview.warnedCap then
			...
			Echo("[Feature Placer] Preview limited to " .. PREVIEW_GHOST_CAP .. " ghosts; all " .. #items .. " features are still placed on click")
		end
	end
	...
	for i = 1, shown do
		...
		if def then
			local handle = draw(def.id, item.x, item.y, item.z, headingToYaw(item.heading), item.pitch or 0, item.roll or 0, item.alpha or GHOST_ALPHA, item.tintR or 1, item.tintG or 1, item.tintB or 1, item.tintAmount or 0, ghosts[n + 1], GHOST_OWNER)
			if handle then
				n = n + 1
				ghosts[n] = handle
			end
...
local function applyMat(m, x, y, z)
	return m[1][1] * x + m[1][2] * y + m[1][3] * z, m[2][1] * x + m[2][2] * y + m[2][3] * z, m[3][1] * x + m[3][2] * y + m[3][3] * z
end
...
local function updateGizmoDrag(mx, my)
	...
	local moved = abs(dx) > 0.01 or abs(dy) > 0.01 or abs(dz) > 0.01 or abs(dPitch) > 1e-4 or abs(dYaw) > 1e-4 or abs(dRoll) > 1e-4
...
local function sendTransforms(targets)
	...
	for i = 1, #targets do
		local t = targets[i]
		batch[#batch + 1] = string.format("%d %.2f %.2f %.2f %.5f %.5f %.5f", t.id, t.x, t.y, t.z, t.pitch, t.yaw, t.roll)
		if #batch >= PLACE_BATCH then
			flush()
		end
	end
...
local function tickSettle()
	...
	for i = 1, #gz.settleTargets do
		...
		local settled = not Spring.ValidFeatureID(t.id) or (x and abs(x - t.x) < SETTLE_EPSILON and abs(y - t.y) < SETTLE_EPSILON and abs(z - t.z) < SETTLE_EPSILON)
		if settled then
		...
		for entry in batchStr:gmatch("[^|]+") do
			...
			if defName and x and z then
				...
				if parts[5] and parts[6] and parts[7] then
					file:write(string.format("\t{ name = %q, x = %s, z = %s, rot = %s, pitch = %s, roll = %s, y = %s },\n", defName, x, z, rot, parts[5], parts[6], parts[7]))
				else
