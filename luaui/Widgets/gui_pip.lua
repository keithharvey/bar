				...
				if #transports > 1 then
					...
					if ux then
						...
						for i = 1, #transports do
							SpringSynced.GiveOrderToUnit(transports[i], cmdID, {ux, uy, uz, smallRadius}, cmdOpts.coded)
						end
					end
				else
					-- Single transport, give direct unit target
					SpringSynced.GiveOrderToUnit(transports[1], cmdID, {id}, cmdOpts.coded)
				end
			end
		else
			...
			if radius and radius > 0 then
				...
				if cmdID == CMD.LOAD_UNITS then
					...
					for i = 1, #selectedUnits do
						local unitDefID = spFunc.GetUnitDefID(selectedUnits[i])
						if unitDefID and cache.isTransport[unitDefID] and (cache.transportCapacity[unitDefID] or 0) > 0 then
							SpringSynced.GiveOrderToUnit(selectedUnits[i], cmdID, {wx, spFunc.GetGroundHeight(wx, wz), wz, radius}, cmdOpts.coded)
						end
					end
				else
...
miscState.startX, _, miscState.startZ = SpringShared.GetTeamStartPosition(Spring.GetMyTeamID())
	...
	for i = 1, #buttons do
		...
		if button.command then
			...
			if button.keybind then
				SpringUnsynced.SendCommands("unbindkeyset " .. button.keybind)
				SpringUnsynced.SendCommands("bind " .. button.keybind .. " " .. button.actionName)
			end
		end
	end
...
function widget:ViewResize()
	...
	if miscState.engineMinimapActive then
		...
		SpringUnsynced.SendCommands(string.format("minimap geometry %d %d %d %d",
...
function widget:Shutdown()
	...
	for i = 1, #buttons do
		...
		if button.command then
			...
			if button.keybind then
				SpringUnsynced.SendCommands("unbind " .. button.keybind .. " " .. button.actionName)
			end
		end
	end
end
	...
	if activeCmdID == CMD_AREA_MEX then
		...
		if metalSpots and not metalMap then
			...
			if mexBuildings then
				if not frameSel then frameSel = SpringUnsynced.GetSelectedUnits() end
...
local function GL4DrawIcons(checkAllyTeamID, selectedSet, trackingSet)
	...
	local bldgBlockWillRebuild = useUnitpics
		...
		or (SpringShared.GetGameFrame() - (gl4Icons._bldgBlockFrame or 0)) >= bldgBlockGameFrameLimit
	for i = 1, unitCount do
...
local function DrawUnitsAndFeatures(cachedSelectedUnits)
	...
	if config.drawComNametags and cameraState.zoom >= config.comNametagZoomThreshold then
		...
		local cappedIconRadius = math.min(iconRadiusZoomDistMult,
			SpringUnsynced.GetConfigFloat("MinimapIconScale", 3.5) * (mapInfo.mapSizeX * mapInfo.mapSizeZ / 40000) ^ 0.25 * math.sqrt(0.95) * resScale)
...
function widget:Update(dt)
	...
	if not gameHasStarted and not isMinimapMode then
		...
		if not isSpec and not interactionState.trackingPlayerID then
			local newX, _, newZ = SpringShared.GetTeamStartPosition(Spring.GetMyTeamID())
