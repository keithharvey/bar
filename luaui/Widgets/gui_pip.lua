...
config = {
	...
	minimapModeMaxWidth = 0.25, -- Max width as fraction of screen width
...
local pipR2T = {
	...
	unitsNextUpdateTime = 0, -- Rolling deadline prevents refreshes aliasing onto one render-only frame
...
local miscState = {
	...
	featureRectLastL = nil, -- Last queried feature-rect left (world)
	featureRectLastT = nil, -- Last queried feature-rect top (world)
	featureRectLastR = nil, -- Last queried feature-rect right (world)
...
function pipTV.DirectorTick(dt)
			...
			if otherPip ~= pipNumber then
				...
				if focus.camX and focus.viewRadius then
					...
					if cdistSq < vr * vr then
						...
						if penalized < worstFactor then
							worstFactor = penalized
						end
					end
				end
	...
	if timeSinceSwitch < config.tvSwitchCooldown then
		...
		if dir.currentHotspotX then
			...
			if coordFactor < 0.3 then
				...
				local zoom = config.tvOverviewZoom + (config.tvCloseupZoom - config.tvOverviewZoom) * math.min(1, cooldownWeightForZoom)
		...
		if dir.currentHotspotX then
			return nil
		end -- Still in cooldown, no coord conflict
		return nil
	end
	...
	local zoom = config.tvOverviewZoom + (config.tvCloseupZoom - config.tvOverviewZoom) * math.min(1, weightForZoom)
	zoom = math.min(zoom, config.tvMaxZoom)
...
function pipTV.CameraUpdate(dt)
	if not pipTV.camera.active then
		return
	end
...
local pools = {
	...
	aliveTeammates = {}, -- Reused for GetAliveTeammates in per-frame UI paths
	trackingOutlineColor = { 1, 1, 1, 0.22 }, -- Reused for DrawTrackingIndicators default outline color
	trackingTeamColor = { 1, 1, 1, 0.5 }, -- Reused for DrawTrackingIndicators team outline color
	unitsInRect = {}, -- Reused for tracked-spec filtering in DrawUnitsAndFeatures
	emptySelectionSet = {}, -- Reused empty selected-set sentinel for tracking mode
	comHealthBars = {}, -- Reused commander health bar entries
	cursorArcRanges = {
		{ math.pi / 2 - math.pi / 8, math.pi / 2 + math.pi / 8 },
		{ 0 - math.pi / 8, 0 + math.pi / 8 },
		{ 3 * math.pi / 2 - math.pi / 8, 3 * math.pi / 2 + math.pi / 8 },
		{ math.pi - math.pi / 8, math.pi + math.pi / 8 },
	}, -- Reused ally cursor arc definitions
	liveSet = {}, -- Reused unit-membership set for ghost visibility checks
	scratchQuad = { x1 = 0, y1 = 0, x2 = 0, y2 = 0 }, -- Reused vertices for DrawScratchQuad
	scratchArc = { cx = 0, cy = 0, radius = 0, start = 0, finish = 0, segments = 0 }, -- Reused params for DrawScratchArc
	scratchTexQuad = { l = 0, b = 0, r = 0, t = 0 }, -- Reused bounds for textured quad callback
	lineClamp = { x1 = 0, y1 = 0, x2 = 0, y2 = 0 }, -- Reused line endpoints for BeginEnd line callback
	minimapViewRect = { halfWidth = 0, halfHeight = 0, cx = 0, cy = 0 }, -- Reused DrawInMiniMap chamfer params
	cameraViewVerts = {}, -- Reused flat vertex buffer for DrawCameraViewBounds edge quads
	cameraViewVertexCount = 0,
	cameraViewCache = { valid = false, points = {} }, -- Cached chamfer points and inputs for DrawCameraViewBounds
	unitsR2T = { uW = 0, uH = 0, resScale = 1, currentRotation = 0, pregameIdleInterval = 0.12 }, -- Reused UpdateR2TUnits render context
}
...
local function CanSkipUntrackedSelectionUpdate(playerID)
	return not trackedPlayerSelections[playerID] and interactionState.trackingPlayerID ~= playerID and GetPlayerSelectedUnitsFromWG() ~= nil
end
...
-- Reused scratch vars/callbacks for hot BeginEnd paths (avoids per-iteration closure allocs)
local function DrawScratchQuad()
	local sq = pools.scratchQuad
	glFunc.Vertex(sq.x1, sq.y1)
	glFunc.Vertex(sq.x2, sq.y1)
	glFunc.Vertex(sq.x2, sq.y2)
	glFunc.Vertex(sq.x1, sq.y2)
end

local function DrawScratchArc()
	local sa = pools.scratchArc
	for i = 0, sa.segments do
		local t = i / sa.segments
		local angle = sa.start + (sa.finish - sa.start) * t
		local x = sa.cx + math.cos(angle) * sa.radius
		local y = sa.cy + math.sin(angle) * sa.radius
		glFunc.Vertex(x, y)
	end
end

pools.DrawLineClamp = function()
	local l = pools.lineClamp
	glFunc.Vertex(l.x1, l.y1)
	glFunc.Vertex(l.x2, l.y2)
end

pools.DrawMinimapViewRectLoop = function()
	local s = pools.minimapViewRect
	glFunc.Vertex(-s.halfWidth + s.cx, -s.halfHeight)
	glFunc.Vertex(s.halfWidth - s.cx, -s.halfHeight)
	glFunc.Vertex(s.halfWidth, -s.halfHeight + s.cy)
	glFunc.Vertex(s.halfWidth, s.halfHeight - s.cy)
	glFunc.Vertex(s.halfWidth - s.cx, s.halfHeight)
	glFunc.Vertex(-s.halfWidth + s.cx, s.halfHeight)
	glFunc.Vertex(-s.halfWidth, s.halfHeight - s.cy)
...
pools.CameraViewChamferPoints = function(x1, y1, x2, y2, cornerX, cornerY, chamfer)
...
pools.CameraViewAppendEdgeQuad = function(data, idx, x1, y1, x2, y2, halfW)
...
pools.DrawCameraViewEdgeQuads = function()
	local data = pools.cameraViewVerts
	local n = pools.cameraViewVertexCount
	for i = 1, n, 2 do
		glFunc.Vertex(data[i], data[i + 1])
	end
end
		...
		OnPress = function()
			...
			if state.losViewEnabled then
				...
				if cameraState.mySpecState then
					for k in pairs(ghostBuildings) do
		...
		OnPress = function()
			...
			if miscState.tvEnabled then
				...
				if WG.pipTVFocus then
					...
					if not next(WG.pipTVFocus) then
						WG.pipTVFocus = nil
					end
				end
			end
...
local function CreateLineVBOSet(maxVertices)
	...
	vao:AttachVertexBuffer(vbo)
	local data = {}
	for i = 1, maxVertices * gl4Prim.LINE_STEP do
		data[i] = 0
	end
	return vbo, vao, data
end
...
local function InitGL4Primitives()
	...
	local cData = {}
	for i = 1, gl4Prim.CIRCLE_MAX * gl4Prim.CIRCLE_STEP do
		cData[i] = 0
	end
	...
	local qData = {}
	for i = 1, gl4Prim.QUAD_MAX * gl4Prim.QUAD_STEP do
		qData[i] = 0
	end
	...
	if nanoShader then
		...
		if nanoVao then
			...
			local nanoData = {}
			for i = 1, config.nanoStreamDrawLimit * nano.INSTANCE_STEP do
				nanoData[i] = 0
			end
...
local function DestroyGL4Primitives()
	...
	nano.enabled = false
	if gl4Prim.circles.shader then
		gl.DeleteShader(gl4Prim.circles.shader)
		gl4Prim.circles.shader = nil
	end
	if gl4Prim.quads.shader then
		gl.DeleteShader(gl4Prim.quads.shader)
		gl4Prim.quads.shader = nil
	end
	if gl4Prim.lineShader then
		gl.DeleteShader(gl4Prim.lineShader)
		gl4Prim.lineShader = nil
	end
	if gl4Prim.quadVBO then
		gl4Prim.quadVBO:Delete()
		gl4Prim.quadVBO = nil
	end
...
local function GL4AddCircle(worldX, worldZ, radius, alpha, coreR, coreG, coreB, edgeR, edgeG, edgeB, edgeAlpha, blendMode)
	...
	if c.count >= gl4Prim.CIRCLE_MAX then
		return
	end
	...
	d[off + 6] = coreG
	d[off + 7] = coreB
	d[off + 8] = edgeAlpha or 0
	d[off + 9] = edgeR
	d[off + 10] = edgeG
	d[off + 11] = edgeB
	d[off + 12] = blendMode or 0
	c.count = c.count + 1
end
...
local function GL4AddQuad(worldX, worldZ, halfW, halfH, angleDeg, r, g, b, a, sprite)
	...
	if q.count >= gl4Prim.QUAD_MAX then
		return
	end
...
local function GL4AddLineToCategory(cat, x1, z1, x2, z2, r, g, b, a, r2, g2, b2, a2)
	if cat.count + 2 > gl4Prim.LINE_MAX then
		return
	end
...
local function ClampDimensionsToScreen(dims)
	if not dims or not dims.l or not dims.r or not dims.b or not dims.t then
		return
	end
...
local function PipToWorldCoords(mx, my)
	...
	return render.world.l + (render.world.r - render.world.l) * normX, render.world.b + (render.world.t - render.world.b) * normY
end

IsInsidePipMap = function(mx, my)
	return not uiState.inMinMode and mx >= render.dim.l and mx <= render.dim.r and my >= render.dim.b and my <= render.dim.t
end
...
local function WorldToPipCoords(wx, wz)
	...
	return wtp.offsetX + wx * wtp.scaleX, wtp.offsetZ + wz * wtp.scaleZ
end
...
local function GroundTextureVertices()
	glFunc.TexCoord(render.ground.coord.l, render.ground.coord.b)
	glFunc.Vertex(render.ground.view.l, render.ground.view.b)
	glFunc.TexCoord(render.ground.coord.r, render.ground.coord.b)
	glFunc.Vertex(render.ground.view.r, render.ground.view.b)
	glFunc.TexCoord(render.ground.coord.r, render.ground.coord.t)
	glFunc.Vertex(render.ground.view.r, render.ground.view.t)
	glFunc.TexCoord(render.ground.coord.l, render.ground.coord.t)
	glFunc.Vertex(render.ground.view.l, render.ground.view.t)
end
...
local function DrawFeature(fID, noTextures)
	...
	if not fDefID or cache.noModelFeatures[fDefID] then
		return
	end

	-- Skip unreclaimable features (decorative objects with no resources)
	if config.hideUnreclaimableFeatures and cache.unreclaimableFeatures[fDefID] then
		return
	end
	...
	if not fx then
		return
	end -- Early exit if position is invalid
...
local function DrawProjectile(pID, pDefID)
	...
	if pDefID then
				...
				if ox then
					...
					for i = 1, numSegments do
						...
						if gl4Prim.enabled then
							...
							GL4AddGlowLine(prevX, prevZ, segX, segZ, colorData[1], colorData[2], colorData[3], 0.4 * avgBrightness)
							GL4AddCoreLine(prevX, prevZ, segX, segZ, coreR, coreG, coreB, 0.98 * avgBrightness)
						end
					...
					if gl4Prim.enabled then
						...
						GL4AddCircle(tx, tz, glowRadius, alpha * 0.5, r * 0.7, g * 0.7, b * 0.8, r * 0.4, g * 0.4, b * 0.5, 0, 0)
						local coreRadius = baseRadius * 0.4
						GL4AddCircle(tx, tz, coreRadius, alpha * 0.95, 1, 1, 1, r * 0.8, g * 0.8, b, alpha * 0.5, 0)
					end
		...
		if cache.weaponIsFlame[pDefID] then
			...
			if gl4Prim.enabled then
				GL4AddQuad(px + offsetX, pz - offsetZ, particleSize, particleSize, v3 * 18, mMin(1, r * 1.2), mMin(1, g * 1.2), mMin(1, b * 1.2), alpha, atlasSprites.flame)
			end
		...
		if showTrail then
			do
				...
				if not trail then
					...
					trail = { positions = {}, head = 0, count = 0, maxLen = trailLen, lastUpdate = 0, isStarburst = isStarburst, isAA = isAA, isTorpedo = isTorpedo, isParalyze = isParalyze, isJuno = isJuno, size = missileSize, speed = projSpeed, trailLife = trailLife }
				...
				if now - trail.lastUpdate >= trailUpdateInterval then
					...
					if trail.head > maxTrailLength then
						trail.head = 1
					end
				...
				if trailCount >= 2 then
					...
					glFunc.BeginEnd(glConst.LINES, function()
						for i = 0, trailCount - 2 do
							...
							local idx1 = head - i
							if idx1 < 1 then
								idx1 = idx1 + maxTrailLength
							end
							local idx2 = head - i - 1
							if idx2 < 1 then
								idx2 = idx2 + maxTrailLength
							end
	...
	if pDefID and cache.weaponIsBlaster[pDefID] then
		...
		GL4AddQuad(px, pz, width * 1.6, height * 1.1, angle, color[1], color[2], color[3], color[4], atlasSprites.blaster)
	elseif pDefID and cache.weaponIsMissile[pDefID] then
		...
		if not mc then
			mc = cache.missileColors[0]
		end -- fallback default
		...
		GL4AddQuad(px + sinA * spriteCenterFwd, pz + cosA * spriteCenterFwd, width * 3.37, height * 0.83, angle, bodyR, bodyG, bodyB, color[4], atlasSprites.missile)
	elseif pDefID and cache.weaponIsBomb[pDefID] then
		...
		GL4AddQuad(px + sinA * spriteCenterFwd, pz + cosA * spriteCenterFwd, bombW, bombH * 0.615, bombAngle, 0.73, 0.73, 0.71, 1.0, atlasSprites.bomb)
	elseif pDefID and cache.weaponIsPlasma[pDefID] then
		...
		GL4AddQuad(px, pz, radius * 1.35, radius * 1.35, 0, plasmaR, plasmaG, plasmaB, color[4], atlasSprites.plasma)
		...
		if trailColor then
			...
			if not trail then
				...
				trail = { positions = {}, head = 0, count = 0, maxLen = 6, lastUpdate = 0, trailLifeFrames = trailLifeFrames, updateInterval = trailUpdateInterval, size = cache.weaponSize[pDefID] or 1 }
			...
			if gameFrame - trail.lastUpdate >= trail.updateInterval then
				trail.head = trail.head + 1
				if trail.head > trail.maxLen then
					trail.head = 1
				end
				local pos = trail.positions[trail.head]
				if pos then
				...
				if trail.count < trail.maxLen then
					trail.count = trail.count + 1
				end
			end
			...
			if trailCount >= 2 then
				...
				local baseWidth = mMin(radius * wtp.scaleX * 2.8, 18 * resScale)
				if baseWidth < 2.0 * resScale then
					baseWidth = 2.0 * resScale
				end
				local minSegW = 1.35 * resScale
				local segDecay = 1.0
		...
		elseif beamLosAlly and not spFunc.IsPosInLos(beam.ox, 0, beam.oz, beamLosAlly) then
			...
			if beam.isLightning then
				...
				for j = 1, #beam.segments do
					...
					if seg.x < beamMinX then
						beamMinX = seg.x
					end
					if seg.x > beamMaxX then
						beamMaxX = seg.x
					end
					if seg.z < beamMinZ then
						beamMinZ = seg.z
					end
					if seg.z > beamMaxZ then
						beamMaxZ = seg.z
					end
				end
			else
			...
			if beamMaxX < worldLeft - beamMargin or beamMinX > worldRight + beamMargin or beamMaxZ < worldTop - beamMargin or beamMinZ > worldBottom + beamMargin then
				i = i + 1
			else
...
local function DrawSeismicPings()
	...
	while i <= n do
		...
		if age > pingLifetime then
			...
			elseif ping.x + ping.maxRadius < worldLeft or ping.x - ping.maxRadius > worldRight or ping.z + ping.maxRadius < worldTop or ping.z - ping.maxRadius > worldBottom then
				i = i + 1
			else
	...
	while i <= n do
		...
		if not explosion or not explosion.x then
			...
			elseif expLosAlly and not spFunc.IsPosInLos(explosion.x, 0, explosion.z, expLosAlly) then
				...
				if explosion.x + explosionRadius < worldLeft or explosion.x - explosionRadius > worldRight or explosion.z + explosionRadius < worldTop or explosion.z - explosionRadius > worldBottom then
					...
					if explosion.isLightning then
						...
						if gl4Prim.enabled then
							...
							GL4AddCircle(explosion.x, explosion.z, glowRadius, glowAlpha, r * 0.7, g * 0.7, b * 0.8, r * 0.4, g * 0.4, b * 0.5, 0, 0)
							local coreAlpha = alpha * 0.95
							local edgeAlpha = alpha * 0.5
							local coreRadius = baseRadius * 0.4
							GL4AddCircle(explosion.x, explosion.z, coreRadius, coreAlpha, 1, 1, 1, r * 0.8, g * 0.8, b, edgeAlpha, 0)
							...
							for k = 1, #explosion.particles do
								...
								if particle.vx and particle.vz and particleLife and particleLife > 0 then
									...
									if particleProgress < 1 then
										...
										GL4AddNormLine(explosion.x + particle.x - sparkDirX, explosion.z - particle.z + sparkDirZ, explosion.x + particle.x + sparkDirX, explosion.z - particle.z - sparkDirZ, r, g, b, sparkAlpha)
									end
								end
							end
						end
						...
						if gl4Prim.enabled then
							...
							GL4AddCircle(explosion.x, explosion.z, baseRadius, coreAlpha, r, g, b, r * 0.4, g * 0.25, b * 0.15, edgeAlpha * 0.5, 0)
							...
							GL4AddCircle(explosion.x, explosion.z, coreRadius, innerAlpha, cr, cg, cb, r, g, b, coreAlpha * 0.8, 0)
							...
							if actualProgress < 0.2 then
								...
								if explosion.isJuno then
									GL4AddCircle(explosion.x, explosion.z, flashRadius, flashAlpha, 0.7, 1, 0.75, 0.4, 0.85, 0.5, flashAlpha * 0.4, 0)
								elseif explosion.isParalyze then
									GL4AddCircle(explosion.x, explosion.z, flashRadius, flashAlpha, 0.85, 0.9, 1, 0.65, 0.75, 1, flashAlpha * 0.4, 0)
								else
									GL4AddCircle(explosion.x, explosion.z, flashRadius, flashAlpha, 1, 1, 1, 1, 0.95, 0.7, flashAlpha * 0.35, 0)
								end
							end
							...
							if explosion.radius > 150 and actualProgress < 0.5 then
								...
								GL4AddCircle(explosion.x, explosion.z, flashRadius, flashAlpha, 1, 1, 1, 1, 0.95, 0.8, flashAlpha * 0.4, 0)
							end
							...
							if explosion.isBigFlash and actualProgress < 0.75 then
								...
								GL4AddCircle(explosion.x, explosion.z, flashR, fa * 0.85, 1, 1, 1, 1, 0.97, 0.92, fa * 0.25, 0)
								-- Inner bright core: tight, fades faster
								if ft < 0.45 then
									local cft = ft / 0.45
									local cfa = (1 - cft * cft) * 0.95
									local coreR = effectiveRadius * (0.4 + cft * 0.6)
									GL4AddCircle(explosion.x, explosion.z, coreR, cfa, 1, 1, 1, 1, 1, 0.97, cfa * 0.5, 0)
								end
							end
						end
					end
		...
		if explosion and explosion.x and not explosion.isLightning then
			...
			if expLosAlly and not spFunc.IsPosInLos(explosion.x, 0, explosion.z, expLosAlly) then
				...
				if age <= lifetime then
					...
					local effectiveRadius = explosion.radius
					if effectiveRadius > 80 then
						effectiveRadius = effectiveRadius * 0.75
					end
					if explosion.isUnitExplosion then
						effectiveRadius = effectiveRadius * 1.33
					end
					...
					GL4AddCircle(explosion.x, explosion.z, baseRadius, overlayAlpha, r, g, b, r * 0.55, g * 0.4, b * 0.2, overlayAlpha * 0.5, 0)
					...
					if explosion.isBigFlash and age / lifetime < 0.75 then
						...
						GL4AddCircle(explosion.x, explosion.z, flashR, flashAlpha, 1, 1, 1, 0.95, 0.93, 0.88, flashAlpha * 0.2, 0)
					end
				end
...
function widget:ViewResize()
	...
	if isMinimapMode then
		...
		Spring.SendCommands(string.format("minimap geometry %d %d %d %d", math.floor(render.dim.l), math.floor(render.vsy - render.dim.t), math.floor(render.dim.r - render.dim.l), math.floor(render.dim.t - render.dim.b)))
	else
		...
		if uiState.savedDimensions.l and uiState.savedDimensions.r and uiState.savedDimensions.b and uiState.savedDimensions.t and oldVsx > 0 and oldVsy > 0 then
		...
		if uiState.inMinMode then
			...
			local dimsValid = render.dim.l
				and render.dim.r
				and render.dim.b
				and render.dim.t
				and oldVsx > 0
				and oldVsy > 0
				and (render.dim.r - render.dim.l) >= minSize
				and (render.dim.t - render.dim.b) >= minSize
				and render.dim.r > minSize -- Not stuck at bottom-left
	...
	local minSize = math.floor(config.minPanelSize * render.widgetScale)
	local savedDimsValid = uiState.savedDimensions.l and uiState.savedDimensions.r and uiState.savedDimensions.b and uiState.savedDimensions.t and (uiState.savedDimensions.r - uiState.savedDimensions.l) >= minSize and (uiState.savedDimensions.t - uiState.savedDimensions.b) >= minSize and uiState.savedDimensions.r > minSize and uiState.savedDimensions.t > minSize
...
function widget:SetConfigData(data)
	if not data or not data.gameID then
		return
	end -- prevent loading empty/corrupted data
	...
	if data.pl and data.pr and data.pb and data.pt then
		...
		if isValidNumber(data.pl, 0, 1) and isValidNumber(data.pr, 0, 1) and isValidNumber(data.pb, 0, 1) and isValidNumber(data.pt, 0, 1) and data.pl < data.pr and data.pb < data.pt then -- Ensure left < right and bottom < top
			...
			if windowWidth >= minSize and windowHeight >= minSize and tempL >= 0 and tempR <= render.vsx and tempB >= 0 and tempT <= render.vsy then
				uiState.savedDimensions = {
					l = tempL,
					r = tempR,
					b = tempB,
	...
	if data.showUnitpics ~= nil then
		config.showUnitpics = data.showUnitpics
	end
	if data.explosionOverlay ~= nil then
		config.explosionOverlay = data.explosionOverlay
	end
	if data.explosionOverlayAlpha then
		config.explosionOverlayAlpha = data.explosionOverlayAlpha
	end
	if data.hideAICommands ~= nil then
		config.hideAICommands = data.hideAICommands
	end
	if data.healthDarkenMax ~= nil then
		config.healthDarkenMax = data.healthDarkenMax
	end
	if data.drawComHealthBars ~= nil then
		config.drawComHealthBars = data.drawComHealthBars
	end
	if data.activityFocusEnabled ~= nil then
		miscState.activityFocusEnabled = data.activityFocusEnabled
	end
	if data.activityFocusIgnoreSpectators ~= nil then
		config.activityFocusIgnoreSpectators = data.activityFocusIgnoreSpectators
	end
	--if data.showTrackedPlayerCursor ~= nil then config.showTrackedPlayerCursor = data.showTrackedPlayerCursor end
	if data.engineMinimapFallback ~= nil then
		config.engineMinimapFallback = data.engineMinimapFallback
	end
	if data.engineMinimapExplosionOverlay ~= nil then
		config.engineMinimapExplosionOverlay = data.engineMinimapExplosionOverlay
	end
	if data.engineMinimapFallbackThreshold ~= nil then
		config.engineMinimapFallbackThreshold = data.engineMinimapFallbackThreshold
	end
...
local function DrawCommandFXOverlay()
	if not config.drawCommandFX then
		return
	end
	if commandFX.count == 0 then
		return
	end
	...
	if useGL4 then
		...
		for i = 1, commandFX.count do
			...
			if age < fxDuration then
				...
				GL4AddNormLine(fx.unitX, fx.unitZ, fx.targetX, fx.targetZ, r, g, b, alpha, r, g, b, alpha)
			end
		end
...
local function DrawCommandQueuesOverlay(cachedSelectedUnits)
	...
	if unitCount > 0 then
		unitHash = unitHash + unitsToShow[1]
	end
	if unitCount > 1 then
		unitHash = unitHash + unitsToShow[unitCount]
	end
	if unitCount > 2 then
		unitHash = unitHash + unitsToShow[math.floor(unitCount / 2)]
	end
		...
		for i = 1, unitCount do
			...
			if cached and cached.n > 0 then
				...
				if ux then
					...
					GL4AddNormLine(ux, uz, firstWP[1], firstWP[2], firstColor[1], firstColor[2], firstColor[3], 0.8, firstColor[1], firstColor[2], firstColor[3], 0.8)
					...
					if cached.n > 1 and chainBudget > 0 then
						...
						for j = 2, cached.n do
							...
							if segDedup[dedupKey] ~= segGen then
								...
								GL4AddNormLine(prevWX, prevWZ, cmdX, cmdZ, r, g, b, 0.8, r, g, b, 0.8)
							end
...
local function DrawQueuedBuilds(iconRadiusZoomDistMult, cachedSelectedUnits)
	...
	if selectedCount > 0 then
		selHash = selHash + selectedUnits[1]
	end
	if selectedCount > 1 then
		selHash = selHash + selectedUnits[selectedCount]
	end
	if selectedCount > 2 then
		selHash = selHash + selectedUnits[math.floor(selectedCount / 2)]
	end
	...
	if fullRebuild then
		...
		qbc.buildCount = 0
		qbc.bitmapCount = 0
		for k in pairs(qbc.bitmapIdx) do
			qbc.bitmapIdx[k] = nil
		end
	end
...
local function GL4DrawIcons(checkAllyTeamID, selectedSet, trackingSet)
	...
	local unitpicWarmCandidate = config.showUnitpics and (gl4Icons.unitpicsActive or unitCount <= config.unitpicMaxUnits) and cameraState.targetZoom >= (config.unitpicZoomThreshold - config.unitpicWarmupRange)
	gl4Icons.ResetUnitpicWarmQueue()
	local unitpicEntries = gl4Icons.unitpicEntries
	if not unitpicEntries then
		unitpicEntries = {}
		gl4Icons.unitpicEntries = unitpicEntries
	end
	...
	if useMobileBlockCache then
		...
		if gl4Icons._mobileBlock and gl4Icons._mobileBlockAge < 2 and checkAllyTeamID == gl4Icons._mobileBlockCheckAlly then
			mobileBlockRebuild = false
		else
			gl4Icons._mobileBlockAge = 0
		end
		gl4Icons._slowMobileBlockAge = (gl4Icons._slowMobileBlockAge or 0) + 1
		if gl4Icons._slowMobileBlock and gl4Icons._slowMobileBlockAge < 4 and checkAllyTeamID == gl4Icons._slowMobileBlockCheckAlly then
	...
	local trustBuildingVisibility = unitCount > config.iconCosmeticSkipThreshold and checkAllyTeamID and not state.losViewEnabled and not (interactionState.trackingPlayerID and cameraState.mySpecState)
	...
	local function processUnit(uID, usedEl)
		...
		if not visibleDefID and not isRadar then
			return usedEl
		end
		...
		if not skipCosmetics and not isRadar then
			...
			if stun and not buildStun then
				isStunned = true
			end
		end
		...
		data[off + 2] = uz
		data[off + 3] = uvs[5]
		data[off + 4] = healthPct * 32 + (isRadar and 1 or 0) + (isTakeable and 2 or 0) + (isStunned and 4 or 0) + (trackedSet and trackedSet[uID] and 8 or 0) + (isSelfD and 16 or 0)
		data[off + 5] = uvs[1]
		data[off + 6] = uvs[2]
		data[off + 7] = uvs[3]
		data[off + 8] = uvs[4]
		...
		data[off + 10] = g
		data[off + 11] = b
		data[off + 12] = (uID * 0.37) % 6.2832 + mathFloor(flashFactor * 100) * 7.0
		return usedEl + 1
	end
	...
	if checkAllyTeamID and not skipGhosts then
		...
		if not isLosViewMode then
			liveSet = pools.liveSet
			for unitID in pairs(liveSet) do
				liveSet[unitID] = nil
			end
			...
			if cachedBldgX then
				...
				if not fastPathHandled then
					...
					if not (uTeam == localGaiaTeamID and cache.isCritter[uDefID]) then
						...
						local isImmobile = (cacheIsBuilding[uDefID] and localCantBeTransported[uDefID]) or (cache.isPseudoBuilding[uDefID] and not miscState.transportedUnits[uID])
						if isImmobile then
	...
	local bldgBlockValid = bldgBlock and bCount == gl4Icons._bldgBlockBCount and bldgHash == gl4Icons._bldgBlockHash and checkAllyTeamID == gl4Icons._bldgBlockCheckAlly and (currentGameFrame - (gl4Icons._bldgBlockFrame or 0)) < bldgBlockGameFrameLimit and not useUnitpics and not gl4Icons._bldgBlockBuiltDuringUnitpics

	gl4Icons._bldgRenderFrame = (gl4Icons._bldgRenderFrame or 0) + 1
	if not bldgBlockValid and bldgBlock and bCount > 800 and checkAllyTeamID == gl4Icons._bldgBlockCheckAlly and bCount == gl4Icons._bldgBlockBCount and bldgHash == gl4Icons._bldgBlockHash and not useUnitpics and not gl4Icons._bldgBlockBuiltDuringUnitpics then
		bldgBlockValid = true
	end
		...
		if bldgBlockValid then
			...
			if needOverlay then
				...
				while j + 11 <= blockFloats do
					...
					data[blockOffset + j + 1] = bldgBlock[j + 1]
					data[blockOffset + j + 2] = bldgBlock[j + 2]
					data[blockOffset + j + 3] = bldgBlock[j + 3]
					data[blockOffset + j + 4] = bldgBlock[j + 4]
					data[blockOffset + j + 5] = bldgBlock[j + 5]
					data[blockOffset + j + 6] = bldgBlock[j + 6]
					data[blockOffset + j + 7] = bldgBlock[j + 7]
					data[blockOffset + j + 8] = bldgBlock[j + 8]
					data[blockOffset + j + 9] = bldgBlock[j + 9]
					data[blockOffset + j + 10] = bldgBlock[j + 10]
					data[blockOffset + j + 11] = bldgBlock[j + 11]
					j = j + 12
				end
			...
			if bldgSetChanged then
				...
				for i = 1, bCount do
					bldgSortedCache[i] = buildingIDs[i]
				end
			...
			for j = blockFloats + 1, prevBlockN do
				bldgBlock[j] = nil
			end
	...
	if useMobileBlockCache and not slowMobileBlockRebuild and gl4Icons._slowVboValid and (gl4Icons._slowMobileBlockCount or 0) == gl4Icons._slowVboUsedElements then
	...
	if not slowMobileVboReuse then
		if useMobileBlockCache and not slowMobileBlockRebuild then
			...
			while j + 11 <= blockFloats do
				...
				data[blockOffset + j + 1] = slowBlock[j + 1]
				data[blockOffset + j + 2] = slowBlock[j + 2]
				data[blockOffset + j + 3] = slowBlock[j + 3]
				data[blockOffset + j + 4] = slowBlock[j + 4]
				data[blockOffset + j + 5] = slowBlock[j + 5]
				data[blockOffset + j + 6] = slowBlock[j + 6]
				data[blockOffset + j + 7] = slowBlock[j + 7]
				data[blockOffset + j + 8] = slowBlock[j + 8]
				data[blockOffset + j + 9] = slowBlock[j + 9]
				data[blockOffset + j + 10] = slowBlock[j + 10]
				data[blockOffset + j + 11] = slowBlock[j + 11]
				j = j + 12
			end
			while j <= blockFloats do
				data[blockOffset + j] = slowBlock[j]
	...
	if useMobileBlockCache and not mobileBlockRebuild and gl4Icons._vboValid and (gl4Icons._mobileBlockCount or 0) == gl4Icons._vboUsedElements then
		mobileVboReuse = true
	...
	if not mobileVboReuse then
		if useMobileBlockCache and not mobileBlockRebuild then
			...
			while j + 11 <= blockFloats do
				...
				data[blockOffset + j + 1] = mobileBlock[j + 1]
				data[blockOffset + j + 2] = mobileBlock[j + 2]
				data[blockOffset + j + 3] = mobileBlock[j + 3]
				data[blockOffset + j + 4] = mobileBlock[j + 4]
				data[blockOffset + j + 5] = mobileBlock[j + 5]
				data[blockOffset + j + 6] = mobileBlock[j + 6]
				data[blockOffset + j + 7] = mobileBlock[j + 7]
				data[blockOffset + j + 8] = mobileBlock[j + 8]
				data[blockOffset + j + 9] = mobileBlock[j + 9]
				data[blockOffset + j + 10] = mobileBlock[j + 10]
				data[blockOffset + j + 11] = mobileBlock[j + 11]
				j = j + 12
			end
			while j <= blockFloats do
				data[blockOffset + j] = mobileBlock[j]
...
local function DrawUnitsAndFeatures(cachedSelectedUnits)
	...
	if featureFade > 0 then -- Only draw features if zoom is above threshold
		...
		if not featureQueryNeedsUpdate then
			local movedEnough = not miscState.featureRectLastL or math.abs(featureL - miscState.featureRectLastL) > 64 or math.abs(featureT - miscState.featureRectLastT) > 64 or math.abs(featureR - miscState.featureRectLastR) > 64 or math.abs(featureB - miscState.featureRectLastB) > 64
	...
	if config.drawProjectiles and gameHasStarted then
		...
		do
			...
			for k in pairs(activeTrails) do
				activeTrails[k] = nil
			end
				...
				if trailThreshold > 0 then
					if skipProjectileTrails then
						...
						if visibleProjEMA < trailThreshold * 0.75 then
							...
							for k in pairs(cache.missileTrails) do
								cache.missileTrails[k] = nil
							end
							for k in pairs(cache.plasmaTrails) do
								cache.plasmaTrails[k] = nil
							end
						end
					else
	...
	if config.drawComNametags and cameraState.zoom >= config.comNametagZoomThreshold then
		...
		if config.drawComHealthBars and not unitpicsActive then
						...
						if comHealthBars and inLos then
							...
							if hp and maxHP and maxHP > 0 then
								...
								if hpFrac < 0.99 then
									...
									local hb = comHealthBars[comHealthCount]
									if not hb then
										hb = {}
										comHealthBars[comHealthCount] = hb
									end
									hb.cx = cx
									hb.cy = cy
									hb.half = iconHalf
		...
		if comHealthCount > 0 then
			...
			for hi = 1, comHealthCount do
				...
				pools.scratchQuad.x1 = hb.cx - barW - outl
				pools.scratchQuad.y1 = barY - outl
				pools.scratchQuad.x2 = hb.cx + barW + outl
				pools.scratchQuad.y2 = barY + barH + outl
				glFunc.BeginEnd(GL.QUADS, DrawScratchQuad)
				...
				pools.scratchQuad.x1 = hb.cx - barW
				pools.scratchQuad.y1 = barY
				pools.scratchQuad.x2 = hb.cx + barW
				pools.scratchQuad.y2 = barY + barH
				glFunc.BeginEnd(GL.QUADS, DrawScratchQuad)
				...
				pools.scratchQuad.x1 = hb.cx - barW
				pools.scratchQuad.y1 = barY
				pools.scratchQuad.x2 = hb.cx + fillW
				pools.scratchQuad.y2 = barY + barH
				glFunc.BeginEnd(GL.QUADS, DrawScratchQuad)
			end
			glFunc.Color(1, 1, 1, 1)
		...
		if comHealthBars then
			for i = comHealthCount + 1, #comHealthBars do
				comHealthBars[i] = nil
			end
		end
	...
	if config.showTrackedPlayerCursor and not config.trackedPlayerCursorGroundGlow and WG.allycursors and WG.allycursors.getCursor and interactionState.trackingPlayerID then
		...
		if cursor and isNotIdle then
			...
			if teamID then
				...
				local segments = 24
				local arcs = pools.cursorArcRanges

				-- Draw black outline first (thicker)
				glFunc.Color(0, 0, 0, 0.66)
				glFunc.LineWidth((render.vsy / 500 + 2) * resScale)
				for _, arc in ipairs(arcs) do
					pools.scratchArc.start = arc[1]
					pools.scratchArc.finish = arc[2]
					pools.scratchArc.segments = math.floor(segments / 8)
					pools.scratchArc.cx = cx
					pools.scratchArc.cy = cy
					pools.scratchArc.radius = cursorSize
					glFunc.BeginEnd(GL.LINE_STRIP, DrawScratchArc)
				end
...
local function RenderFrameButtons()
	...
	for i = 1, buttonCount do
		local isActive = (visibleButtons[i].command == "pip_track" and interactionState.areTracking) or (visibleButtons[i].command == "pip_trackplayer" and interactionState.trackingPlayerID) or (visibleButtons[i].command == "pip_view" and state.losViewEnabled) or (visibleButtons[i].command == "pip_activity" and miscState.activityFocusEnabled) or (visibleButtons[i].command == "pip_tv" and miscState.tvEnabled)
...
local function UpdateLavaRenderState()
	local lrs = mapInfo.lavaRenderState
	if not lrs or lrs.gadgetPushed then
		return
...
pools.DrawMapLines = function()
	...
	if useGL4 then
		...
		local needsRebuild = miscState.mapLinesDirty or miscState.mapLinesLastFilterKey ~= filterKey or currentTime >= miscState.mapLinesNextUpdateTime
		...
		if needsRebuild then
			...
			while i <= n do
				...
				if currentTime >= expireTime then
					...
					if expireTime < nextUpdateTime then
...
local function DrawBuildCursorWithRotation()
	...
	if not mouseOverPip then
		...
		if pos then
			...
			if worldTraceX < render.world.l or worldTraceX > render.world.r or worldTraceZ > render.world.b or worldTraceZ < render.world.t then
	...
	for gx = gridLeft, gridRight, cellSize do
		...
		if x1 >= render.dim.l and x1 <= render.dim.r then
			...
			glFunc.BeginEnd(GL.LINES, pools.DrawLineClamp)
		end
	end
...
local function DrawCameraViewBounds()
	...
	if needRebuild then
		...
		local function screenToGround(sx, sy)
			local pxDirX, pxDirY, pxDirZ = Spring.GetPixelDir(sx, sy)
			...
			local scaledDirY = pxDirY * farDist
			local t
			if math.abs(scaledDirY) < 0.001 then
				-- Near-horizontal ray: project to far distance
				negCount = negCount + 1
				return camX + pxDirX * farDist, camZ + pxDirZ * farDist
			end
			...
			return camX + pxDirX * farDist * t, camZ + pxDirZ * farDist * t
		end
		...
		if rotation ~= 0 then
			...
			local dx, dy
			dx, dy = bl_x - centerX, bl_y - centerY
			bl_x, bl_y = centerX + dx * cosA - dy * sinA, centerY + dx * sinA + dy * cosA
			dx, dy = br_x - centerX, br_y - centerY
			br_x, br_y = centerX + dx * cosA - dy * sinA, centerY + dx * sinA + dy * cosA
			dx, dy = tr_x - centerX, tr_y - centerY
			tr_x, tr_y = centerX + dx * cosA - dy * sinA, centerY + dx * sinA + dy * cosA
			dx, dy = tl_x - centerX, tl_y - centerY
			tl_x, tl_y = centerX + dx * cosA - dy * sinA, centerY + dx * sinA + dy * cosA
		end
		...
		bl_x = math.floor(bl_x) + 0.5
		bl_y = math.floor(bl_y) + 0.5
		br_x = math.floor(br_x) + 0.5
		br_y = math.floor(br_y) + 0.5
		tr_x = math.floor(tr_x) + 0.5
		tr_y = math.floor(tr_y) + 0.5
		tl_x = math.floor(tl_x) + 0.5
		tl_y = math.floor(tl_y) + 0.5
		...
		bl_c1x, bl_c1y, bl_c2x, bl_c2y = pools.CameraViewChamferPoints(tl_x, tl_y, br_x, br_y, bl_x, bl_y, chamfer)
		br_c1x, br_c1y, br_c2x, br_c2y = pools.CameraViewChamferPoints(bl_x, bl_y, tr_x, tr_y, br_x, br_y, chamfer)
		tr_c1x, tr_c1y, tr_c2x, tr_c2y = pools.CameraViewChamferPoints(br_x, br_y, tl_x, tl_y, tr_x, tr_y, chamfer)
		tl_c1x, tl_c1y, tl_c2x, tl_c2y = pools.CameraViewChamferPoints(tr_x, tr_y, bl_x, bl_y, tl_x, tl_y, chamfer)

		points[1], points[2], points[3], points[4] = bl_c1x, bl_c1y, bl_c2x, bl_c2y
		points[5], points[6], points[7], points[8] = br_c1x, br_c1y, br_c2x, br_c2y
		points[9], points[10], points[11], points[12] = tr_c1x, tr_c1y, tr_c2x, tr_c2y
		points[13], points[14], points[15], points[16] = tl_c1x, tl_c1y, tl_c2x, tl_c2y

		cache.valid = true
		cache.wcx, cache.wcz, cache.zoom = wcx, wcz, zoom
		cache.camX, cache.camY, cache.camZ = camX, camY, camZ
		cache.worldL, cache.worldR, cache.worldB, cache.worldT = render.world.l, render.world.r, render.world.b, render.world.t
		cache.rotation = rotation
		cache.dirX, cache.dirY, cache.dirZ = dirX, dirY, dirZ
		cache.vsx, cache.vsy = vsx, vsy
		cache.l, cache.r, cache.b, cache.t = render.dim.l, render.dim.r, render.dim.b, render.dim.t
	else
		bl_c1x, bl_c1y, bl_c2x, bl_c2y = points[1], points[2], points[3], points[4]
		br_c1x, br_c1y, br_c2x, br_c2y = points[5], points[6], points[7], points[8]
		tr_c1x, tr_c1y, tr_c2x, tr_c2y = points[9], points[10], points[11], points[12]
		tl_c1x, tl_c1y, tl_c2x, tl_c2y = points[13], points[14], points[15], points[16]
	end

	local cvVerts = pools.cameraViewVerts
	local idx
	local resScale = render.contentScale or 1
	...
	idx = 1
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, bl_c1x, bl_c1y, bl_c2x, bl_c2y, shadowHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, bl_c2x, bl_c2y, br_c1x, br_c1y, shadowHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, br_c1x, br_c1y, br_c2x, br_c2y, shadowHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, br_c2x, br_c2y, tr_c1x, tr_c1y, shadowHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tr_c1x, tr_c1y, tr_c2x, tr_c2y, shadowHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tr_c2x, tr_c2y, tl_c1x, tl_c1y, shadowHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tl_c1x, tl_c1y, tl_c2x, tl_c2y, shadowHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tl_c2x, tl_c2y, bl_c1x, bl_c1y, shadowHalfW)
	pools.cameraViewVertexCount = idx - 1
	glFunc.Color(0, 0, 0, 0.35)
	glFunc.BeginEnd(GL.QUADS, pools.DrawCameraViewEdgeQuads)

	-- Draw white line on top
	local whiteHalfW = math.max(0.5, 0.65 * ((vsx + 1000) / 3000) * resScale)
	idx = 1
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, bl_c1x, bl_c1y, bl_c2x, bl_c2y, whiteHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, bl_c2x, bl_c2y, br_c1x, br_c1y, whiteHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, br_c1x, br_c1y, br_c2x, br_c2y, whiteHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, br_c2x, br_c2y, tr_c1x, tr_c1y, whiteHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tr_c1x, tr_c1y, tr_c2x, tr_c2y, whiteHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tr_c2x, tr_c2y, tl_c1x, tl_c1y, whiteHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tl_c1x, tl_c1y, tl_c2x, tl_c2y, whiteHalfW)
	idx = pools.CameraViewAppendEdgeQuad(cvVerts, idx, tl_c2x, tl_c2y, bl_c1x, bl_c1y, whiteHalfW)
	pools.cameraViewVertexCount = idx - 1
...
local function UpdateMapRulerTexture()
	...
	local cacheKey = string.format("%d_%d_%d_%d_%d_%d_%d_%d_%d", math.floor(render.world.l / 3) * 3, math.floor(render.world.r / 3) * 3, math.floor(render.world.t / 3) * 3, math.floor(render.world.b / 3) * 3, math.floor(render.dim.l / 3) * 3, math.floor(render.dim.r / 3) * 3, math.floor(render.dim.b / 3) * 3, math.floor(render.dim.t / 3) * 3, markSize)
...
DrawTexturedQuad = function(qL, qB, qR, qT)
	if qL == nil then
		...
		-- Scratch-path quads (tracked minimap overlays) use top-origin mapping.
		glFunc.TexCoord(0, 0)
		glFunc.Vertex(qL, qT)
		glFunc.TexCoord(1, 0)
		glFunc.Vertex(qR, qT)
	...
	-- Explicit-arg path (BlitShiftedTexture) keeps original bottom-origin mapping.
	glFunc.TexCoord(0, 0)
	glFunc.Vertex(qL, qB)
	glFunc.TexCoord(1, 0)
	glFunc.Vertex(qR, qB)
...
function gl4Prim.IsNanoStreamUnitVisible(unitID, viewAllyTeamID)
	...
	if allyTeamID == viewAllyTeamID then
		return true
	end
...
function gl4Prim.DrawNanoStreams()
	local nano = gl4Prim.nanoStreams
	if not config.drawNanoStreams or not nano.enabled or cameraState.zoom < config.nanoStreamMinZoom then
		return
	end
...
-- Pre-created R2T draw function for units texture (avoids per-update closure allocation)
local function unitsR2TDraw()
	local u = pools.unitsR2T
	local uW, uH = u.uW, u.uH

	-- Use the rotation captured by UpdateR2TUnits
	render.minimapRotation = u.currentRotation

	glFunc.Translate(-1, -1, 0)
	glFunc.Scale(2 / uW, 2 / uH, 0)

	-- Use separate blend for alpha: color blends normally, alpha accumulates
	-- correctly for later premultiplied compositing (avoids alpha^2 darkening)
	gl.BlendFuncSeparate(GL.SRC_ALPHA, GL.ONE_MINUS_SRC_ALPHA, GL.ONE, GL.ONE_MINUS_SRC_ALPHA)
	...
	-- keeps the margin expansion in world-space bounds.
	render.dim.l, render.dim.b, render.dim.r, render.dim.t = 0, 0, uW, uH
	render.contentScale = u.resScale
	RecalculateWorldCoordinates()
...
local function DrawTrackedPlayerName()
	...
	local needsUpdate = pipR2T.playerNameDlist == nil or pipR2T.playerNameLastPlayerID ~= interactionState.trackingPlayerID or pipR2T.playerNameLastName ~= playerName
...
local function DrawTrackedPlayerResourceBars()
	...
	local currentTime = os.clock()
	local needsTextUpdate = pipR2T.resbarTextDlist == nil or pipR2T.resbarTextLastPlayerID ~= interactionState.trackingPlayerID or (currentTime - pipR2T.resbarTextLastUpdate) >= pipR2T.resbarTextUpdateRate
...
local function DrawTrackedPlayerMinimap()
	...
	glFunc.LineWidth(3)
	pools.scratchQuad.x1 = viewL
	pools.scratchQuad.y1 = viewB
	pools.scratchQuad.x2 = viewR
	pools.scratchQuad.y2 = viewT
	glFunc.BeginEnd(GL.LINE_LOOP, DrawScratchQuad)
...
local function UpdateR2TUnits(currentTime, pipUpdateInterval, pipWidth, pipHeight)
	...
	local effectiveUpdateInterval = pipUpdateInterval
	-- In pregame, expensive-layer content is usually static. Keep updates responsive while
	-- moving/zooming, but avoid high-frequency refreshes when view state is effectively idle.
	if Spring.GetGameFrame and Spring.GetGameFrame() <= 0 and pipR2T.unitsTex and not pipR2T.unitsNeedsUpdate and not rotChanged and not sizeChanged and not driftForced then
		local desiredZoom = cameraState.targetZoom or cameraState.zoom
		local storedZoom = pipR2T.unitsZoom or desiredZoom
		local baseZoom = (storedZoom ~= 0 and storedZoom) or desiredZoom
		local zoomDelta = math.abs(desiredZoom - storedZoom) / math.max(baseZoom, 0.0001)
		local cameraIdle = not interactionState.areIncreasingZoom and not interactionState.areDecreasingZoom and not interactionState.arePanning and not interactionState.areCentering
		local wideZoom = desiredZoom <= config.pipZoomThresholdMin
		if cameraIdle and wideZoom and zoomDelta < 0.01 then
			local idleInterval = pools.unitsR2T.pregameIdleInterval
			if effectiveUpdateInterval == 0 or effectiveUpdateInterval < idleInterval then
				effectiveUpdateInterval = idleInterval
			end
		end
	end

	local timerDue = effectiveUpdateInterval == 0 or pipR2T.unitsNextUpdateTime <= 0 or (effectiveUpdateInterval > 0 and currentTime >= pipR2T.unitsNextUpdateTime)
	local shouldUpdate = pipR2T.unitsNeedsUpdate or rotChanged or driftForced or (sizeChanged and not uiState.areResizing) or timerDue
	...
	if pipR2T.unitsTex then
		...
		uctx.uW = uW
		uctx.uH = uH
		uctx.resScale = resScale
		uctx.currentRotation = currentRotation

		gl.R2tHelper.RenderToTexture(pipR2T.unitsTex, unitsR2TDraw, true)
		pipR2T.unitsLastUpdateTime = currentTime
		if effectiveUpdateInterval > 0 then
			if timerDue and pipR2T.unitsNextUpdateTime > 0 then
				pipR2T.unitsNextUpdateTime = math.max(pipR2T.unitsNextUpdateTime + effectiveUpdateInterval, currentTime)
			else
				pipR2T.unitsNextUpdateTime = currentTime + effectiveUpdateInterval
			end
		else
			pipR2T.unitsNextUpdateTime = currentTime
		end
...
local function UpdateR2TCheapLayers(currentTime, pipUpdateInterval, pipWidth, pipHeight)
	...
	-- Check if should update based on time
	-- During resize, sizeChanged defers to throttle instead of forcing immediate update
	local timerDue = pipUpdateInterval == 0 or pipR2T.contentNextUpdateTime <= 0 or (pipUpdateInterval > 0 and currentTime >= pipR2T.contentNextUpdateTime)
	local shouldUpdate = pipR2T.contentNeedsUpdate or rotChanged or driftForced or (sizeChanged and not uiState.areResizing) or timerDue
	...
	if pipR2T.contentTex then
		...
		if pipUpdateInterval > 0 then
			if timerDue and pipR2T.contentNextUpdateTime > 0 then
				pipR2T.contentNextUpdateTime = math.max(pipR2T.contentNextUpdateTime + pipUpdateInterval, currentTime)
			else
				pipR2T.contentNextUpdateTime = currentTime + pipUpdateInterval
			end
		else
			pipR2T.contentNextUpdateTime = currentTime
		end
...
local function DrawTrackingIndicators()
	if interactionState.areTracking and #interactionState.areTracking > 0 then
		...
		local outlineColor = pools.trackingOutlineColor
		glFunc.Color(1, 1, 1, 0.22)
		render.RectRoundOutline(render.dim.l, render.dim.b, render.dim.r, render.dim.t, render.elementCorner * 0.5, lineWidth, 1, 1, 1, 1, outlineColor, outlineColor)
	end
	...
	if interactionState.trackingPlayerID then
		...
		if teamID then
			...
			local teamColor = pools.trackingTeamColor
			teamColor[1], teamColor[2], teamColor[3], teamColor[4] = r, g, b, 0.5
			glFunc.Color(r, g, b, 0.5)
			render.RectRoundOutline(render.dim.l, render.dim.b, render.dim.r, render.dim.t, render.elementCorner * 0.5, lineWidth, 1, 1, 1, 1, teamColor, teamColor)
		end
	end
end
...
pools.RunDeferredPipMaintenance = function(dt)
	...
	do
		...
		local n = 0
		for _ in pairs(ghostBuildings) do
			n = n + 1
		end
		if n > maxGhosts then
...
function widget:DrawScreen()
	...
	if sameGameFrameAsLastDraw then
		miscState.pipRenderOnlyDraw = (miscState.pipRenderOnlyDraw or 0) + 1
	else
		if miscState.pipLastDrawGameFrame ~= nil then
			local observedRenderOnlyDraws = miscState.pipRenderOnlyDraw or 0
			local availableRenderOnlyDraws = miscState.pipAvailableRenderOnlyDraws or 1
			if observedRenderOnlyDraws < availableRenderOnlyDraws then
				miscState.pipAvailableRenderOnlyDraws = observedRenderOnlyDraws
				miscState.pipHigherRenderOnlyDrawStreak = 0
			elseif observedRenderOnlyDraws > availableRenderOnlyDraws then
				local higherDrawStreak = (miscState.pipHigherRenderOnlyDrawStreak or 0) + 1
				if higherDrawStreak >= 3 then
					miscState.pipAvailableRenderOnlyDraws = availableRenderOnlyDraws + 1
					higherDrawStreak = 0
				end
				miscState.pipHigherRenderOnlyDrawStreak = higherDrawStreak
			else
				miscState.pipHigherRenderOnlyDrawStreak = 0
			end

			local gameFrameDelta = drawGameFrame - miscState.pipLastDrawGameFrame
			if gameFrameDelta > 0 and miscState.pipLastGameFrameDrawTime then
				local measuredInterval = (drawTime - miscState.pipLastGameFrameDrawTime) / gameFrameDelta
				if measuredInterval > 0 and measuredInterval < 0.25 then
					local previousInterval = miscState.pipGameFrameInterval or (1 / 30)
					miscState.pipGameFrameInterval = previousInterval + (measuredInterval - previousInterval) * 0.15
				end
			end
		end
		miscState.pipRenderOnlyDraw = 0
		miscState.pipLastGameFrameDrawTime = drawTime
	end
	...
	local fallbackUnitThreshold = miscState.engineMinimapActive and (config.engineMinimapFallbackThreshold * 0.95) or config.engineMinimapFallbackThreshold
	local rawUseEngineMinimapFallback = isMinimapMode and config.engineMinimapFallback and #miscState.pipUnits > fallbackUnitThreshold and IsAtMinimumZoom(cameraState.zoom) and IsAtMinimumZoom(cameraState.targetZoom) and not interactionState.trackingPlayerID and not miscState.tvEnabled
	...
	if gl.R2tHelper and not useEngineMinimapFallback then
		...
		if not urgentR2TUpdate then
			...
			if runR2TUpdates and sameGameFrameAsLastDraw then
				local availableRenderOnlyDraws = miscState.pipAvailableRenderOnlyDraws or 1
				if availableRenderOnlyDraws > 1 then
					local gameFrameInterval = miscState.pipGameFrameInterval or (1 / 30)
					local updateSlotsNeeded = math.ceil(dynamicUpdateRate * gameFrameInterval)
					updateSlotsNeeded = math.max(1, math.min(updateSlotsNeeded, availableRenderOnlyDraws))
					local firstUpdateSlot = availableRenderOnlyDraws - updateSlotsNeeded + 1
					if miscState.pipRenderOnlyDraw < firstUpdateSlot then
						runR2TUpdates = false
					end
				end
			end
		...
		do
			...
			local nowUseUnitpics = aboveUnitpicThreshold and (gl4Icons.unitpicsActive or #miscState.pipUnits <= config.unitpicMaxUnits)
		...
		if runR2TUpdates then
			...
			do
				local unitsTimerDue = pipUpdateInterval == 0 or pipR2T.unitsNextUpdateTime <= 0 or currentTime >= pipR2T.unitsNextUpdateTime
				local contentTimerDue = pipUpdateInterval == 0 or pipR2T.contentNextUpdateTime <= 0 or currentTime >= pipR2T.contentNextUpdateTime
				local r2tWorkDue = pipR2T.forceRefreshFrames > 0 or pipR2T.unitsNeedsUpdate or pipR2T.contentNeedsUpdate or not pipR2T.unitsTex or not pipR2T.contentTex or unitsTimerDue or contentTimerDue
		...
		local maskNeedsUpdate = (math.floor(pipWidth) ~= pipR2T.contentMaskLastWidth or math.floor(pipHeight) ~= pipR2T.contentMaskLastHeight or math.floor(render.dim.l) ~= pipR2T.contentMaskLastL or math.floor(render.dim.b) ~= pipR2T.contentMaskLastB)
		...
		if pipR2T.contentTex then
			...
			BlitShiftedTexture(pipR2T.contentTex, pipR2T.contentTexWidth, pipR2T.contentTexHeight, pipR2T.contentWcx, pipR2T.contentWcz, pipR2T.contentZoom, pipR2T.contentRotation)
			...
			if pipR2T.unitsTex then
				...
				BlitShiftedTexture(pipR2T.unitsTex, pipR2T.unitsTexWidth, pipR2T.unitsTexHeight, pipR2T.unitsWcx, pipR2T.unitsWcz, pipR2T.unitsZoom, pipR2T.unitsRotation)
			end
			...
			if isMinimapMode and WG.minimap and widgetHandler and widgetHandler.DrawInMiniMapList then
				...
				for _, w in ipairs(widgetHandler.DrawInMiniMapList) do
					if w ~= widget then -- Don't recursively call ourselves
						...
						gl.Viewport(overlayViewportX, overlayViewportY, overlayViewportWidth, overlayViewportHeight)
	...
	if gl.R2tHelper then
		...
		if not isMinimapMode or config.minimapModeShowMinimizeButton then -- In minimap mode, only show if configured
			...
			if mx >= render.dim.r - render.usedButtonSize - render.elementPadding and mx <= render.dim.r - render.elementPadding and my >= render.dim.t - render.usedButtonSize - render.elementPadding and my <= render.dim.t - render.elementPadding then
				hover = true
				if WG.tooltip then
					WG.tooltip.ShowTooltip("pip" .. pipNumber, BAR.I18N(isMinimapMode and "ui.pip.minimap_minimize" or "ui.pip.minimize"), nil, nil, nil)
				end
				glFunc.Color(1, 1, 1, 0.12)
				glFunc.Texture(false)
	...
	if not uiState.isAnimating then
		...
		if config.showPipFps then
			...
			if config.showPipTimers then
				fpsText = fpsText .. string.format("\ntotal %.1fms  items %d\nfeat %.2f  proj %.2f\nexpl %.2f  icons %.2f", perfTimers.total * 1000, perfTimers.itemCount, perfTimers.features * 1000, perfTimers.projectiles * 1000, perfTimers.explosions * 1000, perfTimers.icons * 1000)
			end
...
function widget:DrawInMiniMap(minimapWidth, minimapHeight)
	...
	if not config.showViewRectangleOnMinimap then
		return
	end
	...
	local cx = chamfer
	local cy = chamfer
	pools.minimapViewRect.halfWidth = halfWidth
	pools.minimapViewRect.halfHeight = halfHeight
	pools.minimapViewRect.cx = cx
	pools.minimapViewRect.cy = cy

	-- draw dark outline
	glFunc.Color(0, 0, 0, 0.4)
	glFunc.LineWidth(outlinewidth)
	glFunc.BeginEnd(GL.LINE_LOOP, pools.DrawMinimapViewRectLoop)
...
function widget:Update(dt)
	...
	local mouseMovedSignificantly = math.abs(mx - interactionState.lastHoverCheckX) > mouseMoveThreshold or math.abs(my - interactionState.lastHoverCheckY) > mouseMoveThreshold
	...
	if not lastSelectionCount then
		lastSelectionCount = 0
	end
	if not lastTrackingState then
		lastTrackingState = false
	end
	if not lastPlayerTrackingState then
		lastPlayerTrackingState = false
	end
	...
	if miscState.tvEnabled and pipTV.camera.active and not miscState.isGameOver and gameHasStarted then
		local userInteracting = interactionState.arePanning or interactionState.areIncreasingZoom or interactionState.areDecreasingZoom or interactionState.isMouseOverPip
	...
	if zoomNeedsUpdate or centerNeedsUpdate then
		...
		if zoomNeedsUpdate then
			...
			local zoomMin = GetEffectiveZoomMin()
			if cameraState.zoom < zoomMin then
				cameraState.zoom = zoomMin
			end
			if cameraState.targetZoom < zoomMin then
				cameraState.targetZoom = zoomMin
			end
		end
...
RebuildAllUnitsCache = function()
	local allUnits = Spring.GetAllUnits()
	local cacheList = miscState.allUnitsCache
	local cacheIndex = miscState.allUnitsIndex

	for i = #cacheList, 1, -1 do
		cacheList[i] = nil
	end
	for unitID in pairs(cacheIndex) do
		cacheIndex[unitID] = nil
	end
	...
	miscState.allUnitsDirty = false
end

AddUnitToAllUnitsCache = function(unitID)
	local cacheIndex = miscState.allUnitsIndex
	if cacheIndex[unitID] then
		return
	end
	local cacheList = miscState.allUnitsCache
	cacheList[#cacheList + 1] = unitID
	cacheIndex[unitID] = #cacheList
end

RemoveUnitFromAllUnitsCache = function(unitID)
	local cacheIndex = miscState.allUnitsIndex
	local idx = cacheIndex[unitID]
	if not idx then
		return
	end

	local cacheList = miscState.allUnitsCache
	local last = #cacheList
	if idx ~= last then
		local movedID = cacheList[last]
		cacheList[idx] = movedID
		cacheIndex[movedID] = idx
	end
	cacheList[last] = nil
	cacheIndex[unitID] = nil
end
...
local function CreateIconShatter(unitID, unitDefID, unitTeam, unitVelX, unitVelZ)
	...
	if not gl4Prim.enabled or #miscState.pipUnits >= 1000 then
		return
	end
	-- Performance: limit max simultaneous shatters
	if #cache.iconShatters >= cache.maxIconShatters then
		return
	end

	-- Only shatter if unit has an icon
	if not cache.unitIcon[unitDefID] then
		return
	end
	...
	if ux < render.world.l - 600 or ux > render.world.r + 600 or uz < render.world.t - 600 or uz > render.world.b + 600 then
		return
	end
...
function widget:UnitDestroyed(unitID, unitDefID, unitTeam)
	RemoveUnitFromAllUnitsCache(unitID)
	...
	if miscState.tvEnabled and unitDefID then
		...
		if weight > 0.2 then
			...
			if x then
				pipTV.AddEvent(x, z, weight, "death")
			end
		end
	end
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
	...
	if miscState.tvEnabled and unitDefID then
		...
		if cost >= config.tvUnitFinishedCostThreshold then
			...
			local x, _, z = spFunc.GetUnitBasePosition(unitID)
			if x then
				pipTV.AddEvent(x, z, weight, "finished")
			end
		end
	end
end
...
function widget:UnitDamaged(unitID, unitDefID, unitTeam, damage, paralyzer)
	if uiState.inMinMode then
		return
	end -- Skip damage flash + TV events when not visible
	if damage <= 0 then
		return
	end
	if paralyzer then
	...
	if miscState.tvEnabled then
		...
		if weight > 0.1 then
			local x, _, z = spFunc.GetUnitBasePosition(unitID)
			if x then
				pipTV.AddEvent(x, z, weight, "combat")
			end
		end
	end
end
...
function widget:UnitEnteredLos(unitID, unitTeam)
	...
	if not cache.isBuilding[unitDefID] then
		return
	end
	local x, _, z = spFunc.GetUnitBasePosition(unitID)
	if not x then
...
function widget:VisibleExplosion(px, py, pz, weaponID, ownerID)
	...
	if not config.drawExplosions then
		return
	end
...
pools.EraseMapLinesAt = function(x, z, radius)
	...
	for i = #lines, 1, -1 do
		local line = lines[i]
		if x >= line.minX - radius and x <= line.maxX + radius and z >= line.minZ - radius and z <= line.maxZ + radius then
...
function widget:MapDrawCmd(playerID, cmdType, mx, my, mz, a, b, c)
	...
	elseif cmdType == "line" then
		if not config.showMapDrawings or a == nil or c == nil then
			return false
		end
		local playerInfo = pools.GetMapDrawPlayerInfo(playerID)
		if not playerInfo or playerInfo.isSpec then
			return false
		end
	...
	elseif cmdType == "erase" then
		if not config.showMapDrawings then
			return false
		end
		local playerInfo = pools.GetMapDrawPlayerInfo(playerID)
		if not playerInfo or playerInfo.isSpec then
...
function widget:IsAbove(mx, my)
	...
	if select(7, spFunc.GetMouseState()) then
		return false
	end
	-- Guard against uninitialized render dimensions
	if not render.dim.l or not render.dim.r or not render.dim.b or not render.dim.t then
	...
	if uiState.isAnimating then
		...
		if uiState.inMinMode then
			...
			return mx >= math.min(render.dim.l, uiState.minModeL) and mx <= math.max(render.dim.r, uiState.minModeL + math.floor(render.usedButtonSize * config.maximizeSizemult)) and my >= math.min(render.dim.b, uiState.minModeB) and my <= math.max(render.dim.t, uiState.minModeB + math.floor(render.usedButtonSize * config.maximizeSizemult))
		else
