...
local function isValidBuildmenuGeometry()
	return isValidRect(backgroundRect[1], backgroundRect[2], backgroundRect[3], backgroundRect[4]) and isFiniteNumber(width) and isFiniteNumber(height) and width > 0 and height > 0 and isFiniteNumber(cellSize) and cellSize > 0 and isFiniteNumber(cellPadding) and isFiniteNumber(iconPadding) and cellSize > ((cellPadding + iconPadding) * 2)
end
...
local function drawCell(cellRectID, usedZoom, cellColor, disabled, underConstruction)
	...
	UiUnit(iconX1, iconY1, iconX2, iconY2, cornerSize, 1, 1, 1, 1, usedZoom, nil, disabled and 0 or nil, unitTexture, showRadarIcon and ((units.unitIconType[uDefID] and iconTypes[units.unitIconType[uDefID]]) and ":l" .. texprefix .. ":" .. iconTypes[units.unitIconType[uDefID]] or nil) or nil, showGroupIcon and (groups[units.unitGroup[uDefID]] and ":l" .. texprefix .. ":" .. groups[units.unitGroup[uDefID]] or nil) or nil, { units.unitMetalCost[uDefID], units.unitEnergyCost[uDefID] }, tonumber(cmds[cellRectID].params[1]))
	tracy.ZoneEnd()
	...
	if cellColor then
		...
		UiUnit(cellRects[cellRectID][1] + cellPadding + iconPadding, cellRects[cellRectID][2] + cellPadding + iconPadding, cellRects[cellRectID][3] - cellPadding - iconPadding, cellRects[cellRectID][4] - cellPadding - iconPadding, cornerSize, 1, 1, 1, 1, usedZoom)
	...
	if cmds[cellRectID].params[1] then
		...
		font2:Print("\255\190\255\190" .. cmds[cellRectID].params[1], cellRects[cellRectID][1] + cellPadding + math_floor(cellInnerSize * 0.96) - pad2, cellRects[cellRectID][2] + cellPadding + math_floor(cellInnerSize * 0.735) - pad2, cellInnerSize * 0.29, "ro")
	end
	...
	if quotaInfo and quotaInfo.quota ~= 0 then
		...
		font2:Print("\255\255\130\190" .. quotaText, cellRects[cellRectID][1] + cellPadding + math_floor(cellInnerSize * 0.96) - pad2, cellRects[cellRectID][2] + cellPadding + (math_floor(cellInnerSize * 0.365) - font2:GetTextHeight(quotaInfo.quota) * quotaFontSize) / 2, quotaFontSize, "ro")
	end
	tracy.ZoneEnd()
	return true
end
...
local function drawHighlights()
	if highlightCount == 0 or not next(highlights) then
		return
	end
	local now = os_clock()
	for uDefID, hl in pairs(highlights) do
		...
		if rect then
			...
			WG.FlowUI.Draw.RectRoundOutline(x1 + thickness, y1 + thickness, x2 - thickness, y2 - thickness, math_max(0, cs - thickness), glowWidth, 1, 1, 1, 1, { r, g, b, glowAlpha }, { r, g, b, 0 })
					...
					if cmd and math_isInRect(x, y, cellRect[1], cellRect[2], cellRect[3], cellRect[4]) then
						...
						if WG["tooltip"] and not meta then
							...
							if units.unitRestricted[uDefID] then
								text = BAR.I18N("ui.buildMenu.disabled", { unit = unitTranslatedHumanName[uDefID], textColor = textColor, warnColor = "\255\166\166\166" })
							else
		...
		if not WG["topbar"] or not WG["topbar"].showingQuit() then
			if math_isInRect(x, y, backgroundRect[1], backgroundRect[2], backgroundRect[3], backgroundRect[4]) then
				...
				if paginatorHovered then
					if WG["tooltip"] then
						local text = "\255\240\240\240" .. (paginatorHovered == 1 and BAR.I18N("ui.buildMenu.previousPage") or BAR.I18N("ui.buildMenu.nextPage"))
...
function widget:GameFrame(n)
	if n % 15 ~= 0 or selectedFactoryCount == 0 then
		return
	end
...
local function buildUnitHandler(_, _, _, data)
	...
	if not unitName[startDefID] or not comBuildOptions[unitName[startDefID]] or not comBuildOptions[unitName[startDefID]][data.unitDefID] then
		return
	end
	...
	if buildCycleIndex > buildCycleCount then
		buildCycleIndex = 1
	end
