	...
	WG.TerraformBrush = {
		...
		listHeightmaps = function()
			...
			for _, path in ipairs(files) do
				...
				local isThisMap = (base == mapPrefix) or (stamp ~= nil) or (base:match("^" .. escaped .. " s%d+$") ~= nil)
				if isThisMap then
					local label
...
function widget:DrawWorld()
	...
	if gridOverlay and not Spring.IsGUIHidden() and not (WG.TerraformCapture and WG.TerraformCapture.isBusy and WG.TerraformCapture.isBusy()) then
	...
	local suppressBrush = extraState.symmetryPlacingOrigin or extraState.symmetryDraggingOrigin or extraState.symmetryHoveringOrigin or Spring.IsGUIHidden() or (WG.TerraformCapture and WG.TerraformCapture.isBusy and WG.TerraformCapture.isBusy()) or (WG.MapLabels and WG.MapLabels.shouldSuppressBrush and WG.MapLabels.shouldSuppressBrush())
