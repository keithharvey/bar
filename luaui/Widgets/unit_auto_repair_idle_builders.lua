...
for unitDefID, unitDef in pairs(UnitDefs) do
	if unitDef.isBuilder and (unitDef.canAssist or unitDef.canResurrect) and unitDef.canMove and unitDef.speed > 0 and not unitDef.isFactory then
...
function widget:GameFrame(frame)
	...
	for reclaimerID, targetID in pairs(activeReclaimers) do
		if spGetUnitTeam(reclaimerID) ~= myTeam and (not isUnitAlive(reclaimerID) or not isReclaimingUnit(reclaimerID, targetID)) then
	...
	for builderID, homePos in pairs(idleBuilders) do
		...
		elseif spGetUnitIsBeingBuilt(builderID) then
			...
			for _, candidateID in ipairs(nearbyUnits) do
				if candidateID ~= builderID and not reclaimBlacklist[candidateID] and not spGetUnitIsBeingBuilt(candidateID) then
