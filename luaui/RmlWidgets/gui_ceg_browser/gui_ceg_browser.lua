...
local function LoadAllCEGs()
	...
	table.sort(names)
	allCEGs = names
	spEcho(string.format("[CEG Browser] Loaded %d CEG names from %d file(s) (ok=%d err=%d).", #allCEGs, #files, okCount, errCount))
end
...
local function LoadAllSounds()
	...
	for _, dir in ipairs(dirs) do
		for _, mode in ipairs(modes) do
			...
			for _, path in ipairs(files) do
				local logical = path:gsub("^sounds/", ""):gsub("%.[%a%d]+$", "")
...
local function SpawnPattern(name, wx, wz)
	...
	local msg = string.format("cegtest:%s:%d:%d:%d:%d:%s:%d", name, x, z, cnt, spac, pat, ho)
...
local function SpawnPatternMulti(names, wx, wz)
	...
	local msg = string.format("cegtest_multi:%s:%d:%d:%d:%d:%s:%d", table.concat(names, ","), x, z, cnt, spac, pat, ho)
	if selectedImpactSound and selectedImpactSound ~= "" then
		msg = msg .. "|impactSound=" .. selectedImpactSound
	end
	spSendLuaRulesMsg(msg)
...
local function BuildItems(filterText, letter)
	...
	for _, name in ipairs(allCEGs) do
		...
		if (lf == "" or low:sub(1, 1) == lf) and (q == "" or low:find(q, 1, true)) then
			out[#out + 1] = { name = name }
		end
	end
	...
	local function dump(t, indent, depth)
		...
		for _, k in ipairs(sortedKeys(t)) do
			...
			if type(v) == "table" then
				...
				if not addLine(indent .. tostring(k) .. ": " .. vs) then
					return
				end
			end
		end
	...
	if ctrl then
		if tbl[name] then
			...
			if selType == "trail" then
				selectedCEG = ""
				for k in pairs(selectedCEGs) do
					selectedCEG = k
					break
				end
			end
...
function widget:MousePress(x, y, button)
	...
	if fireArmed then
		...
		local impactList = {}
		local muzzleList = {}
		for name in pairs(selectedCEGs) do
			trailList[#trailList + 1] = name
		end
		for name in pairs(selectedImpactCEGs) do
			impactList[#impactList + 1] = name
		end
		for name in pairs(selectedMuzzleCEGs) do
			muzzleList[#muzzleList + 1] = name
		end
		...
		for _, trailName in ipairs(trailList) do
			local msg = string.format("cegproj:%s:%s:%d:%d:%d:%d:%d:%.2f", trailName, table.concat(impactList, ","), wx, wz, math.floor(projYaw + 0.5), math.floor(projPitch + 0.5), math.floor(projSpeed + 0.5), projGravity)
			msg = msg .. string.format("|ttl=%.2f|airburst=%d", projTTL, projAirburst and 1 or 0)
			...
			msg = msg .. string.format("|ofs=%d,%d", math.floor(projFwdOffset + 0.5), math.floor(projUpOffset + 0.5))
			spSendLuaRulesMsg(msg)
		end
		return true
	end
...
local init_model = {
	...
	setSpawnCount = function(ev)
		...
		if dm_handle then
			dm_handle.spawnCount = spawnCount
		end
	end,
	...
	setSpacing = function(ev)
		...
		if dm_handle then
			dm_handle.spacingValue = spacingValue
		end
	end,
	...
	setHeightOffset = function(ev)
		...
		if dm_handle then
			dm_handle.heightOffset = heightOffset
		end
	end,
	...
	setProjFwdOffset = function(ev)
		...
		if dm_handle then
			dm_handle.projFwdOffset = projFwdOffset
		end
	end,
	...
	setProjUpOffset = function(ev)
		...
		if dm_handle then
			dm_handle.projUpOffset = projUpOffset
		end
	end,
	...
	toggleAirburst = function(ev)
		...
		if dm_handle then
			dm_handle.projAirburst = projAirburst and 1 or 0
		end
	end,
	...
	sendCheat = function(ev)
		local isSP = BAR.Utilities and BAR.Utilities.Gametype and BAR.Utilities.Gametype.IsSinglePlayer and BAR.Utilities.Gametype.IsSinglePlayer()
...
function widget:Update()
	...
	if hoveredCEG ~= lastHovered then
		...
		local rml
		if pre and suf then
			rml = '<span class="tooltip-prefix">' .. pre .. "</span>" .. '<span class="tooltip-suffix">' .. suf .. "</span>"
		else
	...
	if leftPx + 260 > vsx then
		leftPx = mx - 270
	end
	leftPx = math.max(0, leftPx)
	topPx = math.max(0, topPx)
	tooltipEl:SetAttribute("style", string.format("left: %.2fvw; top: %.2fvh;", (leftPx / vsx) * 100, (topPx / vsy) * 100))
...
function widget:Initialize()
	...
	if soundListEl then
		soundListEl:AddEventListener("mousedown", function(ev)
			local p = ev and ev.parameters
			if not p or p.button ~= 1 then
				return
			end -- RMB = button 1 in RmlUi
			if soundHoveredName == "" then
				return
			end
...
function widget:Shutdown()
	...
	if ctx2 and dm_handle then
		ctx2:RemoveDataModel(MODEL_NAME)
	end
	document = nil
	dm_handle = nil
end
