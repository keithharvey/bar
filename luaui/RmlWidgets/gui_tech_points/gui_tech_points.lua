...
function widget:GetInfo()
	return {
		...
		desc = "Displays Keystone progress toward tech level thresholds",
		author = "SethDGamre",
		date = "2025-10",
...
---@type { techLevel: number, points: number, t2Threshold: number, t3Threshold: number }
local cached = {
	techLevel = 1,
	points = 0,
	t2Threshold = 2,
...
---@type { rmlContext: any, document: any, dmHandle: any, lastUpdate: number, fillElement: any, levelElement: any, countElement: any, popupElement: any, milestoneElements: any[], previousTechLevel: number, popupEndTime: number, gameStartTime: number?, initialPopupShown: boolean, myTeamID: integer?, lastCacheTime: number }
local widgetState = {
	rmlContext = nil,
	document = nil,
	dmHandle = nil,
...
local function refreshCache()
	...
	if rawT2 then
		cached.t2Threshold = tonumber(rawT2) or cached.t2Threshold
	end
	if rawT3 then
		cached.t3Threshold = tonumber(rawT3) or cached.t3Threshold
	end
end
...
local function getProgressPercent()
	...
	elseif cached.techLevel >= 2 then
		if cached.t3Threshold <= 0 then
			return 100
		end
		return math.min(100, (cached.points / cached.t3Threshold) * 100)
	else
		if cached.t2Threshold <= 0 then
			return 100
		end
...
local prevLevel, prevCount, prevProgress = 0, -1, -1 ---@type number, number, number
...
local function updateUI()
	...
	if widgetState.dmHandle then
		...
		widgetState.dmHandle.keystoneCount = points
...
function widget:Initialize()
	...
	widgetState.countElement = document:GetElementById("tech-keystone-count")
