...
local function serializeLabels(nodate)
	...
	for _, L in ipairs(state.labels) do
		out[#out + 1] = string.format("\t\t{ id = %d, x = %.1f, z = %.1f, color = %q, author = %q, created = %q, ts = %d, text = %q },\n", L.id, L.x, L.z, L.color, L.author, L.created or "", L.ts or 0, L.text or "")
	end
	out[#out + 1] = "\t},\n}\n"
...
local function refreshPopupContent()
	...
	if state.popupTextEl then
		state.popupTextEl.inner_rml = (L.text ~= "" and renderMultiline(L.text)) or '<span style="color: #6b7280;">(empty label)</span>'
	end
...
local function rebuildUi()
	...
	if layer then
		...
		for _, L in ipairs(state.labels) do
			...
			prev.inner_rml = '<div class="ml-preview-author">' .. escapeRml(displayAuthor(L)) .. '</div><div class="ml-preview-text">' .. snippet(L.text) .. "</div>"
			el:AppendChild(prev)
	...
	if listEl then
		...
		for _, L in ipairs(sorted) do
			...
			body.inner_rml = '<div class="ml-item-author">' .. escapeRml(displayAuthor(L)) .. '</div><div class="ml-item-snippet">' .. snippet(L.text) .. "</div>"
...
function widget:Update()
	...
	if not hoverId then
		...
		local overWindow = (state.windowOpen and cursorOverRect(state.rootEl, mx, myTop)) or (state.openLabelId and cursorOverRect(state.popupEl, mx, myTop))
...
function widget:Initialize()
	...
	WG.MapLabels = {
		...
		getLabels = function()
			local out = {}
			for i, L in ipairs(state.labels) do
				out[i] = { id = L.id, x = L.x, z = L.z, color = L.color, author = L.author, created = L.created, text = L.text }
			end
			return out
		end,
