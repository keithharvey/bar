...
function M.attach(doc, ctx)
	...
	local function findPreview(ddsBase)
		...
		for key, path in pairs(previewFiles) do
			...
			if #stripped >= 6 and (ddsBase:find(stripped:sub(1, math.min(#stripped, 12)), 1, true) or stripped:find(ddsBase:sub(1, math.min(#ddsBase, 12)), 1, true)) then
				return path
			end
			if #ddsStripped >= 4 and (key:find(ddsStripped:sub(1, math.min(#ddsStripped, 10)), 1, true) or ddsStripped:find(stripped:sub(1, math.min(#stripped, 10)), 1, true)) then
				return path
			end
		end
		return nil
	...
	if gridEl then
		...
		if #ddsFiles == 0 then
			gridEl.inner_rml = '<div class="text-sm text-keybind" style="padding: 16dp; text-align: center; font-size: 1rem; line-height: 1.5;">' .. "No skybox textures found.<br/>" .. 'Get skyboxes from the Discord <span style="color: #fdc04c;">#mapping</span> channel pins<br/>' .. "and place .dds files in your BAR data folder:<br/>" .. '<span style="color: #9ca3af;">Beyond-All-Reason/data/Terraform Brush/SkyBoxes/</span>' .. "</div>"
		end
	end
			...
			while true do
				...
				if hex then
					...
					swatch:AddEventListener("click", function(event)
						...
						envSetSlider("slider-env-" .. cfg.sliderPrefix .. "-r", "lbl-env-" .. cfg.sliderPrefix .. "-r", math.floor(hr * 1000 + 0.5), string.format("%.2f", hr))
						envSetSlider("slider-env-" .. cfg.sliderPrefix .. "-g", "lbl-env-" .. cfg.sliderPrefix .. "-g", math.floor(hg * 1000 + 0.5), string.format("%.2f", hg))
						envSetSlider("slider-env-" .. cfg.sliderPrefix .. "-b", "lbl-env-" .. cfg.sliderPrefix .. "-b", math.floor(hb * 1000 + 0.5), string.format("%.2f", hb))
						refreshPreview()
						event:StopPropagation()
					end, false)
				end
	...
	local function envCheckbox(btnId, initVal, onChange)
		...
		local state = initVal
		btn:SetAttribute("src", state and "/luaui/images/terraform_brush/check_on.png" or "/luaui/images/terraform_brush/check_off.png")
	...
	local function envSectionToggle(toggleBtnId, toggleImgId, sectionId, defaultExpanded)
		...
		toggleImg:SetAttribute("src", expanded and "/luaui/images/terraform_brush/minus.png" or "/luaui/images/terraform_brush/plus.png")
	...
	do
		...
		wireVisibilityChip("fp-filter-chip-slope", "fp-smart-slope-content", getFP, { "avoidCliffs", "preferSlopes" }, "avoidCliffs")
		wireVisibilityChip("fp-filter-chip-altitude", "fp-smart-altitude-content", getFP, { "altMinEnable", "altMaxEnable" }, "altMinEnable")
		...
		wireVisibilityChip("sp-filter-chip-slope", "sp-smart-slope-content", getSP, { "avoidCliffs", "preferSlopes" }, "avoidCliffs")
		wireVisibilityChip("sp-filter-chip-altitude", "sp-smart-altitude-content", getSP, { "altMinEnable", "altMaxEnable" }, "altMinEnable")
	...
	do
		...
		for _, wc in ipairs(waterColorParams) do
			for i, suffix in ipairs({ "r", "g", "b" }) do
				envSlider("slider-env-wc-" .. wc.prefix .. "-" .. suffix, "lbl-env-wc-" .. wc.prefix .. "-" .. suffix, function(v)
					return v / 1000
				end, function()
		...
		local function applyWaterTypePreset(colors)
			...
			for _, entry in ipairs(sliderMap) do
				...
				for i, s in ipairs({ "r", "g", "b" }) do
					envSetSlider("slider-env-wc-" .. prefix .. "-" .. s, "lbl-env-wc-" .. prefix .. "-" .. s, math.floor((c[i] or 0) * 1000 + 0.5), string.format("%.2f", c[i] or 0))
				end
