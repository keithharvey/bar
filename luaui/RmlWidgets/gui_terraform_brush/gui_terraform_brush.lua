...
local initialModel = {
	...
	onFileOpenProject = function(_event)
		...
		if #projects == 0 then
			listEl.inner_rml = '<div style="padding: 4dp 6dp; font-size: 0.9rem; color: #6b7280;">' .. "No projects found in MapProjects/. Projects saved this session may need an engine restart to appear (VFS folder cache).</div>"
			return
		end
		for _, p in ipairs(projects) do
			local label = string.format('%s&nbsp;&nbsp;<span style="color: #6b7280;">%sx%s&nbsp;&nbsp;%s</span>', p.name or p.slug, tostring(p.size_x or "?"), tostring(p.size_z or "?"), (p.modified or ""):gsub("T", " "):gsub("Z", ""))
			local item = doc:CreateElement("div")
			...
			item:AddEventListener("click", function(ev)
				...
				elseif WG.MapProject.isBusy and WG.MapProject.isBusy() then
					if dm then
						dm.projectOpenHint = "A save or load is already running (see console)."
					end
				elseif not WG.MapProject.open(slug) then
					if dm then
						dm.projectOpenHint = "Could not open '" .. slug .. "' — see console for the reason."
					end
				end
				ev:StopPropagation()
			end, false)
