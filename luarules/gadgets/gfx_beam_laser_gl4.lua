					...
					if vx then
						...
						if visible then
							...
							if spIsAABBInView(mathMin(px, endX) - pad, mathMin(py, endY) - pad, mathMin(pz, endZ) - pad, mathMax(px, endX) + pad, mathMax(py, endY) + pad, mathMax(pz, endZ) + pad) then
								...
								local innerKey, tracked, emitterX, emitterY, emitterZ = resolveBeamEmitter(ownerBeams, ownerID, wDefID, origPx, origPy, origPz, callStamp, "liveStamp", ownerID ~= 0 and cfg.emitterMatchDistSq or HARDPOINT_MATCH_DISTANCE_SQ, ownerID ~= 0 and cfg.claimedMatchDistSq or HARDPOINT_DEDUPE_DISTANCE_SQ)
								if not tracked then
				...
				if cfg then
					...
					if px then
						...
						if vx then
							...
							local innerKey, tracked, emitterX, emitterY, emitterZ = resolveBeamEmitter(ownerBeams, ownerID, wDefID, px, py, pz, n, "simStamp", ownerID ~= 0 and cfg.emitterMatchDistSq or HARDPOINT_MATCH_DISTANCE_SQ, ownerID ~= 0 and cfg.claimedMatchDistSq or HARDPOINT_DEDUPE_DISTANCE_SQ)
							if not tracked then
								if trackedPoolN > 0 then
									tracked = trackedPool[trackedPoolN]
									trackedPool[trackedPoolN] = nil
