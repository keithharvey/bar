...
if gadgetHandler:IsSyncedCode() then
	...
	local function GetMapSquareKey(posx, posz)
		if posx < 0 or posz < 0 or posx > mapSizeX or posz > mapSizeZ then
			return nil
		end
	...
	local function IsPosInScum(unitx, unity, unitz)
		...
		if scumBinID == nil or scumBins[scumBinID] == nil then
			return nil
		end
	...
	local function AddOrUpdateScum(posx, posy, posz, radius, growthrate, scumID)
		...
		if scums[scumID] == nil then
			...
			if debugmode then
				Spring.Echo("Updated scum", scumID, "it was", currentradius, "/", scum.radius, "sized, growing at", growthrate)
			end
		end
		...
		if scum.growthrate < 0 then
			if debugmode then
				Spring.Echo("Removal of scum ID", scumID, "Scheduled for ", deathtime - gf, "from now")
			end
...
elseif not BAR.Utilities.Gametype.IsScavengers() then -- UNSYNCED
	...
	local function initGL4(shaderConfig, DPATname)
		...
		scumVBO.VAO = InstanceVBOTable.makeVAOandAttach(scumVBO.vertexVBO, scumVBO.instanceVBO, scumVBO.indexVBO)
	end
		...
		if not headless then
			pushElementInstance(
				scumVBO, -- push into this Instance VBO Table
				{
					...
					scum.growthrate,
					0,
					0, -- alphastart_alphadecay_heatstart_heatdecay
				},
				scumID, -- this is the key inside the VBO Table, should be unique per unit
				true, -- update existing element
				false
			) -- noupload, dont use unless you know what you are doing and want to batch push/pop
		end
		if scum.growthrate < 0 then
			if debugmode then
				Spring.Echo("Removal of scum ID", scumID, "Scheduled for ", deathtime - gf, "from now")
	...
	local function RemoveScum(instanceID)
		if debugmode then
			Spring.Echo("Removing scum", instanceID)
		end
			...
			for scumID, scum in pairs(scums) do
				if ((scumID % 4) == scumModulo) and scum.growthrate > 0 and not scum.atmaxsize then
					...
					if currentRadius < scum.radius then
						...
						if debugmode then
							Spring.Echo("Scum ID", scumID, "reached max size", currentRadius, ">=", scum.radius)
						end
						scum.atmaxsize = true
					end
				end
