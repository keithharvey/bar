...
local TechBlockingShared = VFS.Include("common/luaUtilities/sharing/tech_blocking_shared.lua")
...
local ContextFactory = VFS.Include("common/luaUtilities/sharing/context_factory.lua")
ContextFactory.registerPolicyContextEnricher(function(ctx, springRepo, senderTeamID)
	...
	local function findNextProgression(baseKey, currentValue, normalize)
		for scanLevel = level + 1, 3 do
			...
			if normalize then
				futureValue = normalize(futureValue)
			end
	...
	if #modes == 0 then
		modes = { ModeEnums.UnitFilterCategory.None }
	end
