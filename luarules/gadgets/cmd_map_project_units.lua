...
local function exportAllUnits()
	...
	for i = 1, #ids do
		...
		if def and x then
			data[#data + 1] = format("%s %.1f %.1f %d %d %d", def.name, x, z, Spring.GetUnitHeading(uid) or 0, Spring.GetUnitTeam(uid) or 0, Spring.GetUnitNeutral(uid) and 1 or 0)
		end
	end
...
local function replaceAllUnits()
	...
	Spring.Echo(string.format("[Map Project Units] loadout applied: %d spawned, %d failed, %d pre-existing removed%s", spawned, failed, removed, remapped > 0 and (", " .. remapped .. " remapped to Gaia (team missing/dead)") or ""))
end
