...
if gadgetHandler:IsSyncedCode() then
		...
		for startUnitDefID in pairs(participatingDefs) do
			if not visited[startUnitDefID] then
				...
				while #stack > 0 do
					...
					if unitLimit and unitLimit > 0 and unitLimit < DEFAULT_MAX_THIS_UNIT then
	...
	local function registerFamilyBuilder(builderID, builderDefID, teamID)
		...
		for i = 1, #buildOptions do
			...
			if familyID and not registeredFamilies[familyID] then
				...
				familyBuilders[teamID][familyID] = familyBuilders[teamID][familyID] or {}
				familyBuilders[teamID][familyID][builderID] = true
	...
	local function reAssignAssists(newUnit, oldUnit)
		...
		for _, unitID in pairs(allUnits) do
			...
			for j = 1, #cmds do
				...
				if
					(unitTargetCommand[cmd.id] or (singleParamUnitTargetCommand[cmd.id] and #params == 1))
					and (params[1] == oldUnit)
				then
					params[1] = newUnit
					local opts = (cmd.options.meta and CMD.OPT_META or 0)
						+ (cmd.options.ctrl and CMD.OPT_CTRL or 0)
						+ (cmd.options.alt and CMD.OPT_ALT or 0)
					Spring.GiveOrderToUnit(
						unitID,
						CMD.INSERT,
						{ cmd.tag, cmd.id, opts, params[1], params[2], params[3] },
						0
					)
	...
	local function skipEvolutions(evolutionCurrent)
		...
		if
			evolutionCurrent.evolution_condition ~= "timer"
			and evolutionCurrent.evolution_condition ~= "timer_global"
		then
		...
		while
			evolution
			and evolution.evolution_condition
			and evolution.evolution_timer
			and evolution.evolution_target
		do
	...
	local function evolve(unitID)
		...
		if
			(
				not evolution.evolution_condition
				or evolution.evolution_condition == "timer"
				or evolution.evolution_condition == "timer_global"
			)
			and evolutionMetaList[newUnitID]
			and evolutionMetaList[newUnitID].timeCreated
			and delayedSeconds > 0
		then
	...
	local function fillToCheckUnitIDs()
		...
		table.sort(toCheckUnitIDs, function(a, b)
	...
	local function isEvolutionTimePassed(evolution, currentTime)
		return (
			evolution.evolution_condition == "timer"
		...
		while lastCheckIndex <= nToCheckUnitIDs and batchSize < clampedBatchSize do
			...
			if
				...
				and (
					...
					or isEvolutionXPPassed(unitID, evolution)
				)
			then
				evolve(unitID)
			end
