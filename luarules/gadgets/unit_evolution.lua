...
if gadgetHandler:IsSyncedCode() then
	...
	local function evolve(unitID)
		...
		if (not evolution.evolution_condition or evolution.evolution_condition == "timer" or evolution.evolution_condition == "timer_global") and evolutionMetaList[newUnitID] and evolutionMetaList[newUnitID].timeCreated and delayedSeconds > 0 then
	...
	local function isEvolutionTimePassed(evolution, currentTime)
		return (evolution.evolution_condition == "timer" and (currentTime - evolution.timeCreated) >= evolution.evolution_timer) or (evolution.evolution_condition == "timer_global" and currentTime >= evolution.evolution_timer)
	end
	...
	function gadget:GameFrame(f)
		...
		while lastCheckIndex <= nToCheckUnitIDs and batchSize < clampedBatchSize do
			...
			if evolution and not combatCheckUpdate(unitID, evolution, currentTime) and not spGetUnitTransporter(unitID) and (isEvolutionTimePassed(evolution, currentTime) or isEvolutionPowerPassed(evolution) or isEvolutionXPPassed(unitID, evolution)) then
				evolve(unitID)
			end
