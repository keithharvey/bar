...
if gadgetHandler:IsSyncedCode() then
	...
	local function isAuthorized(playerID)
		...
		if (_G and _G.permissions and _G.permissions.devhelpers and (accountID and _G.permissions.devhelpers[accountID] or (playername and _G.permissions.devhelpers[playername]))) or (SYNCED and SYNCED.permissions and SYNCED.permissions.devhelpers and (accountID and SYNCED.permissions.devhelpers[accountID] or (playername and SYNCED.permissions.devhelpers[playername]))) then
			return true
		end
	...
	local function SpawnPattern(names, x, z, count, spacing, pat, height)
		...
		elseif pat == "ring" then
			...
			for i = 0, count - 1 do
				...
				SpawnCEGSet(names, x + radius * cos(a), z + radius * sin(a), height)
			end
		elseif pat == "scatter" then
			...
			for i = 1, count do
				...
				local a = 2 * pi * random()
				SpawnCEGSet(names, x + r * cos(a), z + r * sin(a), height)
			end
		end
	end
	...
	local function FireCEGTestProjectile(ceg, impactBlock, muzzleBlock, fireSound, impactSound, x, z, yawDeg, pitchDeg, speed, gravity, ttlFrames, airburst, fwdOfs, upOfs)
		...
		local unitID = spCreateUnit(TEST_UNIT_NAME, unitX, unitY, unitZ, 0, Spring.GetGaiaTeamID())
		if not unitID then
			return
		end
	...
	function gadget:RecvLuaMsg(msg, playerID)
		...
		if msg:sub(1, #PREFIX_PROJ) == PREFIX_PROJ or msg:sub(1, #PREFIX_GROUND) == PREFIX_GROUND then
			...
			if not trailCEG or trailCEG == "" then
				return
			end
		...
		local nameField, xs, zs, cs, ss, pat, hs = body:match("^([^:]+):([^:]+):([^:]+):([^:]+):([^:]+):([^:]+):?(.*)$")
		...
		SpawnPattern(names, tonumber(xs), tonumber(zs), tonumber(cs) or 1, tonumber(ss) or 0, pat, height)
	end
