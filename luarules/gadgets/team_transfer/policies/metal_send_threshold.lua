...
local metalThreshold = enabled and (ServiceRegistry.SpringRepository().GetModOptions().player_metal_send_threshold or 0) or 0
...
GG.TeamTransfer.RegisterInitialize(function()
	...
	for _, teamID in ipairs(teamList) do
		if not springRepo.GetTeamRulesParam(teamID, CUMULATIVE_METAL_PARAM) then
			springRepo.SetTeamRulesParam(teamID, CUMULATIVE_METAL_PARAM, 0)
		end
	end
end)
...
GG.TeamTransfer.RegisterPolicy(SharedEnums.Policies.MetalSendThreshold, function(policy)
	policy.ForAlliedMetalTransfers.Use(function(ctx)
		...
		if ctx.senderTeamId and ctx.senderTeamId >= 0 then
			cumulativeMetal = springRepo.GetTeamRulesParam(ctx.senderTeamId, CUMULATIVE_METAL_PARAM) or 0
		end
...
GG.TeamTransfer.RegisterPostTransfer(function(transferData)
	if transferData.resource == SharedEnums.ResourceType.METAL then
		...
		if transferData.senderTeamID and transferData.senderTeamID >= 0 then
			local springRepo = ServiceRegistry.SpringRepository()
			local currentCumulative = springRepo.GetTeamRulesParam(transferData.senderTeamID, CUMULATIVE_METAL_PARAM) or 0
			local newCumulative = currentCumulative + (transferData.amount or 0)
			springRepo.SetTeamRulesParam(transferData.senderTeamID, CUMULATIVE_METAL_PARAM, newCumulative)
		else
			ServiceRegistry.SpringRepository().Log("TeamTransfer", "ERROR", "Invalid senderTeamID in PostTransfer: " .. tostring(transferData.senderTeamID))
		end
	end
end)
